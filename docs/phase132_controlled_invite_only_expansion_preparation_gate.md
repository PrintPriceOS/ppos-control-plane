...
* **Phase 128.1 Real DB Persistence (`132.0.12`)**: Under real DB mode, mock/fallback validation passes are completely rejected. Real DB verification requires full signal persistence where `recovered_from_db = true`, `memory_state_detected = false`, and `restart_safe = true` are explicitly written into the database columns (or serialized JSON payloads) as `1` / `0` and verified via a post-insert re-read. Setting `FORCE_REAL_DB_SMOKE=true` enforces that tests must run against a live DB and fails closed with a `REAL_DB_REQUIRED_BUT_UNAVAILABLE` error if the database is offline or unreachable.
