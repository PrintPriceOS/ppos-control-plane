export interface TokenAuthRecord {
  activation_token_auth_id: string;
  ...
  activation_token_auth_status: 'DRAFT' | 'READY_FOR_EVALUATION' | 'EVALUATED' | 'READY_FOR_DECISION' | 'AUTHORIZED' | 'REJECTED' | 'FINALIZED' | 'BLOCKED' | 'FAILED' | 'SUPERSEDED';
  activation_token_auth_result: string | null;
  risk_level: string;
...
export interface TokenAuthRuleCheck {
  rule_id: string;
  activation_token_auth_id: string;
  check_type: string;
  severity: string;
...
export interface TokenAuthEvidence {
  evidence_id: string;
  activation_token_auth_id: string;
...
export interface TokenAuthAuditLog {
