// Phase 162 — Activation Token Redemption Authorization Gate — UI Types

export interface ActivationTokenRedemptionAuthRecord {
  activation_token_redemption_auth_id: string;
  ...
  activation_token_redemption_auth_status:
    | 'DRAFT' | 'READY_FOR_EVALUATION' | 'EVALUATED' | 'READY_FOR_DECISION'
    | 'AUTH_PASSED' | 'AUTH_FAILED' | 'FINALIZED' | 'BLOCKED' | 'FAILED' | 'SUPERSEDED';
  activation_token_redemption_auth_result:
    | 'REDEMPTION_AUTHORIZED_NOT_REDEEMED' | 'REDEMPTION_AUTH_FAILED'
    | 'REDEMPTION_AUTH_BLOCKED_BY_READINESS' | 'REDEMPTION_AUTH_BLOCKED_BY_GUARDRAIL'
    | 'REDEMPTION_AUTH_BLOCKED_BY_HASH_MISMATCH' | 'REDEMPTION_AUTH_BLOCKED_BY_WRITE_SCOPE'
    | 'REDEMPTION_AUTH_BLOCKED_BY_REDEEMABLE_TOKEN' | 'ESCALATE_TO_BOARD_OF_TRUSTEES' | null;
  risk_level: string;
  ...
  redemption_auth_signatures_json: { security_officer_confirmed?: boolean; compliance_officer_confirmed?: boolean; operations_director_confirmed?: boolean };
  redemption_auth_metadata_json: Record<string, unknown>;
  ...
  activation_token_redemption_auth_hash: string | null;
  token_redemption_auth_evidence_pack_hash: string | null;
...
export interface ActivationTokenRedemptionAuthRule {
  rule_id: string;
  activation_token_redemption_auth_id: string;
  check_type: string;
...
export interface RedemptionAuthSignatures {
