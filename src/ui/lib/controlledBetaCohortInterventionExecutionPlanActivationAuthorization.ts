export interface AuthorizationRecord {
  ...
  activation_auth_status: 'DRAFT' | 'READY_FOR_EVALUATION' | 'EVALUATED' | 'READY_FOR_DECISION' | 'AUTHORIZED' | 'REJECTED' | 'BLOCKED' | 'FINALIZED' | 'FAILED' | 'SUPERSEDED';
  activation_auth_result: string | null;
  risk_level: string;
  ...
  source_activation_readiness_hash: string;
  activation_authorization_hash: string | null;
  authorization_evidence_pack_hash: string | null;
...
export interface AuthorizationRuleCheck {
  rule_id: string;
  activation_auth_id: string;
  check_type: string;
  severity: string;
