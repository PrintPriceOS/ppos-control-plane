export interface LockRecord {
  ...
  activation_lock_status: 'DRAFT' | 'READY_FOR_EVALUATION' | 'EVALUATED' | 'READY_FOR_DECISION' | 'LOCKED' | 'FROZEN' | 'FINALIZED' | 'BLOCKED' | 'FAILED' | 'SUPERSEDED';
  activation_lock_result: string | null;
  risk_level: string;
  ...
  lock_summary_json: any;
  impact_review_json: any;
  rollback_review_json: any;
  guardrail_review_json: any;
  lock_rules_json: any;
  lock_blockers_json: any;
  non_execution_attestation_json: any;
  write_scope_attestation_json: any;
  source_activation_authorization_hash: string;
  activation_lock_hash: string | null;
  freeze_package_hash: string | null;
  lock_evidence_pack_hash: string | null;
...
export interface LockRuleCheck {
  rule_id: string;
  activation_lock_id: string;
  check_type: string;
  severity: string;
