  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title={`Tenant: ${tenant.tenant_id}`}>
      <div className="space-y-8 italic-text-off">
        {/* Status Header */}
        <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-none border border-slate-100">
          <div className="w-12 h-12 rounded-none bg-white border border-slate-200 flex items-center justify-center">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-none border border-slate-100 bg-white">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-3 rounded-none hover:bg-slate-50 transition-colors">
                <span className="text-sm font-medium text-slate-700">{flag.label}</span>
                {flag.enforced ? (
                  <ShieldCheckIcon className="w-5 h-5 text-emerald-500" />
                ) : (
                  <div className="w-5 h-5 rounded-none border-2 border-slate-200" />
                )}
              </div>
            ))}
          </div>
