...
import { ControlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealClient } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealClient';
import { TokenRedemptionUnlockSeal, UnlockSealRule } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockSeal';
import { ShieldAlert, CheckCircle, AlertTriangle, CheckSquare, Lock, Eye, AlertOctagon } from 'lucide-react';

export default function ControlledBetaCohortInterventionSimulationExecutionPlanActivationTokenRedemptionUnlockSeal() {
  const { unlockSealId } = useParams<{ unlockSealId: string }>();
  const navigate = useNavigate();
  const client = new ControlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealClient();

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [record, setRecord] = useState<TokenRedemptionUnlockSeal | null>(null);
  const [rules, setRules] = useState<UnlockSealRule[]>([]);

  // 8 Confirmations State
  ...
  const [decisionType, setDecisionType] = useState<'APPROVE_SEAL' | 'REJECT_SEAL' | 'BLOCK' | 'ESCALATE'>('APPROVE_SEAL');

  const loadDetails = async () => {
    if (!unlockSealId) return;
    try {
      setLoading(true);
      const res = await client.getUnlockSealDetails(unlockSealId);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockSeal);
        setRules(res.rules || []);
      }
    } catch (e: any) {
  ...
  const handleEvaluate = async () => {
    if (!record) return;
    try {
      const res = await client.evaluateUnlockSeal(record.activation_token_redemption_unlock_seal_id, {
        ...
        final_review_unlock_readiness_confirmation: readinessConfirmed,
        seal_authenticity_confirmation: authenticityConfirmed
      });
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockSeal);
        setRules(res.rules || []);
  ...
  const handleDecision = async () => {
    if (!record) return;
    try {
      const res = await client.recordDecision(record.activation_token_redemption_unlock_seal_id, decisionType, rationale);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockSeal);
      }
    } catch (e: any) {
  ...
  const handleFinalize = async () => {
    if (!record) return;
    try {
      const res = await client.finalizeUnlockSeal(record.activation_token_redemption_unlock_seal_id);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockSeal);
      }
    } catch (e: any) {
      ...
      <div className="bg-gradient-to-r from-amber-950 via-red-950 to-amber-950 border-b border-amber-800 p-4">
        <div className="max-w-7xl mx-auto flex items-center space-x-3 text-amber-300">
          ...
          <div className="text-sm font-medium">
            ...
            This phase performs unlock readiness seal only. The token is not unlocked, the token is not redeemable, the token is not redeemed. No execution plan is enabled, no jobs or queue dispatches are created. Runtime mutation count remains zero.
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-800 pb-6 gap-4">
          <div>
            <div className="flex items-center space-x-2 text-xs font-mono text-emerald-500 mb-1 uppercase tracking-wider">
              <span>Phase 169</span>
              <span>•</span>
              <span>Unlock Readiness Seal Gate</span>
            </div>
            <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">
              {record.activation_token_redemption_unlock_seal_id}
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Parent Review ID: <span className="font-mono text-slate-300">{record.source_activation_token_redemption_unlock_final_review_id}</span>
            </p>
          </div>
          <div className="flex items-center space-x-3">
            <span className={`px-3 py-1 rounded-full text-xs font-mono font-semibold uppercase tracking-wider ${
              record.unlock_seal_status === 'FINALIZED' ? 'bg-emerald-950 border border-emerald-800 text-emerald-300' :
              record.unlock_seal_status === 'APPROVED' ? 'bg-sky-950 border border-sky-800 text-sky-300' :
              'bg-slate-900 border border-slate-700 text-slate-300'
            }`}>
              {record.unlock_seal_status}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: 'Security Officer Unlock Seal Confirmation', state: securityConfirmed, setter: setSecurityConfirmed },
                  { label: 'Compliance Officer Unlock Seal Confirmation', state: complianceConfirmed, setter: setComplianceConfirmed },
                  { label: 'Operations Director Unlock Seal Confirmation', state: opsDirectorConfirmed, setter: setOpsDirectorConfirmed },
                  { label: 'Rollback Authority Unlock Seal Confirmation', state: rollbackConfirmed, setter: setRollbackConfirmed },
                  ...
                  { label: 'Seal Authenticity Confirmed', state: authenticityConfirmed, setter: setAuthenticityConfirmed },
                ].map((item, idx) => (
                  <label key={idx} className="flex items-center space-x-3 p-3 bg-[#131924] hover:bg-[#1a2230] rounded-lg cursor-pointer border border-slate-800 transition-colors">
                    <input
                      type="checkbox"
                      checked={item.state}
                      disabled={record.unlock_seal_status !== 'DRAFT'}
              ...
              {record.unlock_seal_status === 'DRAFT' && (
                <button
            ...
            {record.unlock_seal_status === 'EVALUATED' && (
              <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
                <h2 className="text-lg font-bold flex items-center space-x-2 border-b border-slate-800 pb-3">
                  <Eye className="w-5 h-5 text-sky-400" />
                  <span>2. Record Readiness Seal Decision</span>
                </h2>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {(['APPROVE_SEAL', 'REJECT_SEAL', 'BLOCK', 'ESCALATE'] as const).map((dec) => (
                        <button
                          key={dec}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Provide reasoning for this readiness seal decision..."
            ...
            {/* Step 3: Finalization */}
            {(record.unlock_seal_status === 'APPROVED' || record.unlock_seal_status === 'REJECTED') && (
              <div className="bg-gradient-to-b from-[#111622] to-[#0b0e16] border border-slate-800 rounded-xl p-6 text-center space-y-4">
                ...
                <h3 className="text-lg font-bold text-slate-200">Pending Readiness Seal Lock Finalization</h3>
                <p className="text-sm text-slate-400 max-w-lg mx-auto">
                  A decision has been recorded. Finalization will seal the Phase 169 evidence pack, establish the lineage hash chain, and freeze the gate state.
                </p>
                <button
          ...
          <div className="space-y-8">
            <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
              <h3 className="text-sm font-mono text-slate-400 uppercase tracking-wider border-b border-slate-800 pb-2">Readiness Seal Ledger</h3>
              <div className="space-y-4">
                {[
