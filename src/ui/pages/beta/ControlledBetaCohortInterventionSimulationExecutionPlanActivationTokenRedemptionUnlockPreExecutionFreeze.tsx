...
import { ControlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreezeClient } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreezeClient';
import { TokenRedemptionUnlockPreExecutionFreeze, UnlockPreExecutionFreezeRule } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreeze';
import { ShieldAlert, CheckCircle, AlertTriangle, CheckSquare, Lock, Eye, AlertOctagon } from 'lucide-react';

export default function ControlledBetaCohortInterventionSimulationExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreeze() {
  const { unlockPreExecutionFreezeId } = useParams<{ unlockPreExecutionFreezeId: string }>();
  const navigate = useNavigate();
  const client = new ControlledBetaCohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreezeClient();

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [record, setRecord] = useState<TokenRedemptionUnlockPreExecutionFreeze | null>(null);
  const [rules, setRules] = useState<UnlockPreExecutionFreezeRule[]>([]);

  // 9 Confirmations State
  ...
  const [decisionType, setDecisionType] = useState<'APPROVE_FREEZE' | 'REJECT_FREEZE' | 'BLOCK' | 'ESCALATE'>('APPROVE_FREEZE');

  const loadDetails = async () => {
    if (!unlockPreExecutionFreezeId) return;
    try {
      setLoading(true);
      const res = await client.getUnlockPreExecutionFreezeDetails(unlockPreExecutionFreezeId);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockPreExecutionFreeze);
        setRules(res.rules || []);
      }
    } catch (e: any) {
  ...
  const handleEvaluate = async () => {
    if (!record) return;
    try {
      const res = await client.evaluateUnlockPreExecutionFreeze(record.activation_token_redemption_unlock_pre_execution_freeze_id, {
        security_officer_unlock_freeze_confirmation: securityConfirmed,
        compliance_officer_unlock_freeze_confirmation: complianceConfirmed,
        operations_director_unlock_freeze_confirmation: opsDirectorConfirmed,
        rollback_authority_unlock_freeze_confirmation: rollbackConfirmed,
  ...
  const handleDecision = async () => {
    if (!record) return;
    try {
      const res = await client.recordDecision(record.activation_token_redemption_unlock_pre_execution_freeze_id, decisionType, rationale);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockPreExecutionFreeze);
      }
    } catch (e: any) {
  ...
  const handleFinalize = async () => {
    if (!record) return;
    try {
      const res = await client.finalizeUnlockPreExecutionFreeze(record.activation_token_redemption_unlock_pre_execution_freeze_id);
      if (res.success) {
        setRecord(res.tokenRedemptionUnlockPreExecutionFreeze);
      }
    } catch (e: any) {
      ...
      <div className="bg-gradient-to-r from-[#1b120c] via-[#241010] to-[#1b120c] border-b border-amber-900/40 p-4">
        <div className="max-w-7xl mx-auto flex items-center space-x-3 text-amber-300">
          ...
          <div className="text-sm font-medium">
            ...
            This phase freezes unlock readiness only. The token is not unlocked. The token is not redeemable. The token is not redeemed. No execution plan is enabled. No jobs or queue dispatches are created. Runtime mutation count remains zero.
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-800 pb-6 gap-4">
          <div>
            <div className="flex items-center space-x-2 text-xs font-mono text-emerald-500 mb-1 uppercase tracking-wider">
              <span>Phase 170</span>
              <span>•</span>
              <span>Unlock Pre-Execution Freeze Gate</span>
            </div>
            <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">
              {record.activation_token_redemption_unlock_pre_execution_freeze_id}
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Parent Seal ID: <span className="font-mono text-slate-300">{record.source_activation_token_redemption_unlock_seal_id}</span>
            </p>
          </div>
          <div className="flex items-center space-x-3">
            <span className={`px-3 py-1 rounded-full text-xs font-mono font-semibold uppercase tracking-wider ${
              record.unlock_pre_execution_freeze_status === 'FINALIZED' ? 'bg-emerald-950 border border-emerald-800 text-emerald-300' :
              record.unlock_pre_execution_freeze_status === 'APPROVED' ? 'bg-sky-950 border border-sky-800 text-sky-300' :
              'bg-slate-900 border border-slate-700 text-slate-300'
            }`}>
              {record.unlock_pre_execution_freeze_status}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: 'Security Officer Unlock Freeze Confirmation', state: securityConfirmed, setter: setSecurityConfirmed },
                  { label: 'Compliance Officer Unlock Freeze Confirmation', state: complianceConfirmed, setter: setComplianceConfirmed },
                  { label: 'Operations Director Unlock Freeze Confirmation', state: opsDirectorConfirmed, setter: setOpsDirectorConfirmed },
                  { label: 'Rollback Authority Unlock Freeze Confirmation', state: rollbackConfirmed, setter: setRollbackConfirmed },
                  ...
                  <label key={idx} className="flex items-center space-x-3 p-3 bg-[#131924] hover:bg-[#1a2230] rounded-lg cursor-pointer border border-slate-800 transition-colors">
                    <input
                      type="checkbox"
                      checked={item.state}
                      disabled={record.unlock_pre_execution_freeze_status !== 'DRAFT'}
              ...
              {record.unlock_pre_execution_freeze_status === 'DRAFT' && (
                <button
            ...
            {record.unlock_pre_execution_freeze_status === 'EVALUATED' && (
              <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
                <h2 className="text-lg font-bold flex items-center space-x-2 border-b border-slate-800 pb-3">
                  <Eye className="w-5 h-5 text-sky-400" />
                  <span>2. Record Pre-Execution Freeze Decision</span>
                </h2>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {(['APPROVE_FREEZE', 'REJECT_FREEZE', 'BLOCK', 'ESCALATE'] as const).map((dec) => (
                        <button
                          key={dec}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Provide reasoning for this pre-execution freeze decision..."
            ...
            {/* Step 3: Finalization */}
            {(record.unlock_pre_execution_freeze_status === 'APPROVED' || record.unlock_pre_execution_freeze_status === 'REJECTED') && (
              <div className="bg-gradient-to-b from-[#111622] to-[#0b0e16] border border-slate-800 rounded-xl p-6 text-center space-y-4">
                ...
                <h3 className="text-lg font-bold text-slate-200">Pending Pre-Execution Freeze Finalization</h3>
                <p className="text-sm text-slate-400 max-w-lg mx-auto">
                  A decision has been recorded. Finalization will seal the Phase 170 evidence pack, establish the lineage hash chain, and freeze the gate state.
                </p>
                <button
          ...
          <div className="space-y-8">
            <div className="bg-[#0e121a] border border-slate-800 rounded-xl p-6 space-y-6">
              <h3 className="text-sm font-mono text-slate-400 uppercase tracking-wider border-b border-slate-800 pb-2">Pre-Execution Freeze Ledger</h3>
              <div className="space-y-4">
                {[
