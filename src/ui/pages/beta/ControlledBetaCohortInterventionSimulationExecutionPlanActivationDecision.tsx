...
import { controlledBetaCohortInterventionExecutionPlanActivationDecisionClient } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationDecisionClient';
import { DecisionRecord, DecisionRuleCheck, DecisionEvidence, DecisionAuditLog } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationDecision';
...
export const ControlledBetaCohortInterventionSimulationExecutionPlanActivationDecision: React.FC = () => {
  const [decisionList, setDecisionList] = useState<DecisionRecord[]>([]);
  const [activeDecisionId, setActiveDecisionId] = useState<string | null>(null);

  // Detail states
  const [decision, setDecision] = useState<DecisionRecord | null>(null);
  const [rules, setRules] = useState<DecisionRuleCheck[]>([]);
  ...
  const [selectedResult, setSelectedResult] = useState('GO_APPROVED_NOT_ACTIVE');
  ...
  const loadList = async () => {
    setLoading(true);
    try {
      const data = await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.getDecisionList();
      setDecisionList(data);
      addLog('Fetched activation decision logs.');
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const loadDetails = async (id: string) => {
    ...
    setError(null);
    try {
      const data = await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.getDecisionDetails(id);
      setDecision(data.decision);
      setRules(data.rules);
      setEvidence(data.evidence);
      setAuditLogs(data.auditLogs);
      setActiveDecisionId(id);
      addLog(`Details loaded for decision record: ${id}`);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const createRecord = async () => {
    if (!lockIdInput.trim()) {
      setError('Activation Lock ID is required');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.createDecision(lockIdInput.trim());
      addLog(`Activation decision draft ${res.activation_decision_id} initialized.`);
      setLockIdInput('');
      await loadList();
      await loadDetails(res.activation_decision_id);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const evaluateRecord = async () => {
    ...
    try {
      ...
      if (overrideHash === 'false') {
        overrides.canary_envelope = { decision_mode: 'EXECUTABLE', allow_real_activation: true }; // triggers failure block
      }
      if (overrideResult) overrides.activation_decision_result = overrideResult;

      await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.evaluateDecision(decision.activation_decision_id, overrides);
      addLog(`Completed rules validation for decision: ${decision.activation_decision_id}`);
      await loadDetails(decision.activation_decision_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  const recordDecision = async () => {
    if (!decision || !decisionRationale.trim()) {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.recordDecision(decision.activation_decision_id, selectedResult, decisionRationale.trim());
  ...
  const finalizeRecord = async () => {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionPlanActivationDecisionClient.finalizeDecision(decision.activation_decision_id);
      addLog(`Finalized decision packet. Evidence Pack v153.0 locked.`);
      await loadDetails(decision.activation_decision_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Inter, system-ui, sans-serif', color: '#1e293b', background: '#f8fafc', minHeight: '100vh' }}>
      <header style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px', fontWeight: 700, letterSpacing: '-0.025em', color: '#0f172a' }}>
            Final Activation Go/No-Go Gate
          </h1>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '14px' }}>
            Phase 153 Gate — Final activation Go/No-Go decision checks while maintaining strict non-active boundaries
          </p>
        </div>
      </header>
      ...
      <div style={{ background: '#fef3c7', borderLeft: '4px solid #d97706', padding: '16px', borderRadius: '4px', marginBottom: '24px' }}>
        <h4 style={{ margin: '0 0 4px 0', color: '#92400e', fontWeight: 600 }}>⚠️ WARNING: GO DECISION ONLY (NON-ACTIVE)</h4>
        <p style={{ margin: 0, color: '#b45309', fontSize: '13px' }}>
          Final GO approval does not activate the execution plan. Final GO approval does not make the plan executable. Final GO approval does not create jobs, dispatch queues, or mutate runtime state. A future activation handoff gate is required before any executable transition can be considered.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px' }}>
        ...
        <div>
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>Create Activation Decision</h3>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                type="text"
                placeholder="Finalized Lock ID..."
                value={lockIdInput}
          ...
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>Decision Records</h3>
            {loading && decisionList.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {decisionList.map((item) => (
                  <div
                    key={item.activation_decision_id}
                    onClick={() => loadDetails(item.activation_decision_id)}
                    style={{
                      padding: '12px',
                      borderRadius: '6px',
                      border: '1px solid',
                      borderColor: activeDecisionId === item.activation_decision_id ? '#3b82f6' : '#e2e8f0',
                      background: activeDecisionId === item.activation_decision_id ? '#eff6ff' : '#f8fafc',
                      cursor: 'pointer',
                      transition: 'all 0.2s'
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 600, color: '#334155' }}>{item.activation_decision_id}</span>
                      <span style={{
                        fontSize: '10px',
                        fontWeight: 700,
                        padding: '2px 6px',
                        borderRadius: '4px',
                        background: STATUS_COLORS[item.activation_decision_status] || '#cbd5e1',
                        color: '#ffffff'
                      }}>{item.activation_decision_status}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Lock ID: {item.source_activation_lock_id}</div>
                    {item.activation_decision_result && (
        ...
        <div>
          {decision ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                  <div>
                    <h2 style={{ margin: 0, fontSize: '20px', color: '#0f172a' }}>Activation Decision: {decision.activation_decision_id}</h2>
                    <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#64748b' }}>
                      Derived from Lock: <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{decision.source_activation_lock_id}</span>
                    </p>
                  </div>
                  <span style={{ padding: '6px 12px', borderRadius: '6px', background: '#f1f5f9', fontSize: '12px', fontWeight: 600 }}>
                    Status: {decision.activation_execution_status}
                  </span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      {decision.package_freeze_status}
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Real Jobs</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      {decision.job_creation_status}
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Decision Result</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#3b82f6', marginTop: '2px' }}>{decision.activation_decision_result || 'PENDING EVALUATION'}</div>
                  </div>
                </div>

                {/* Decision Submit */}
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
                  <h4 style={{ margin: '0 0 12px 0', fontSize: '14px', fontWeight: 600 }}>Log Activation Decision Result</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '290px 1fr', gap: '12px', marginBottom: '12px' }}>
                    ...
                    >
                      <option value="GO_APPROVED_NOT_ACTIVE">Go Approved (Not Active)</option>
                      <option value="NO_GO_NOT_ACTIVE">No-Go (Not Active)</option>
                      <option value="DECISION_BLOCKED_BY_PARENT_LOCK">Blocked by Lock</option>
                      <option value="DECISION_BLOCKED_BY_GUARDRAIL">Blocked by Guardrail</option>
                      <option value="DECISION_BLOCKED_BY_HASH_MISMATCH">Blocked by Hash Mismatch</option>
                      <option value="DECISION_BLOCKED_BY_WRITE_SCOPE">Blocked by Write Scope</option>
                      <option value="DECISION_BLOCKED_BY_EXECUTABLE_FLAG">Blocked by Executable Flag</option>
                      <option value="REQUIRE_LOCK_REVALIDATION">Require Lock Revalidation</option>
                  ...
                  <button
                    ...
                    disabled={decision.activation_decision_status === 'FINALIZED'}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <button
                      ...
                      style={{ padding: '8px 16px', background: '#3b82f6', color: '#ffffff', border: 'none', borderRadius: '4px', fontSize: '13px', cursor: 'pointer', fontWeight: 500 }}
                    >
                      Evaluate Decision & Attenuation
                    </button>
                    <button
                      onClick={finalizeRecord}
                      disabled={!decision.activation_decision_result || decision.activation_decision_status === 'FINALIZED'}
                    ...
                    >
                      Finalize & Lock Decision Package
                    </button>
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px', background: '#faf5ff', padding: '16px', borderRadius: '6px' }}>
                  ...
                  <div style={{ marginTop: '12px' }}>
                    ...
                    >
                      ...
                      <option value="GO_APPROVED_NOT_ACTIVE">Force Go Approved</option>
                      <option value="DECISION_BLOCKED_BY_GUARDRAIL">Force Blocked</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              {evidence && (
                <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: 600 }}>Evidence Package (v153.0)</h3>
            ...
            <div style={{ background: '#ffffff', padding: '48px', borderRadius: '8px', border: '1px solid #e2e8f0', textAlign: 'center', color: '#64748b' }}>
              Select a decision record or enter a finalized activation lock ID to start.
            </div>
          )}
        </div>
      </div>
