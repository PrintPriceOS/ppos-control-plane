...
import { controlledBetaCohortInterventionExecutionEnvelopeClient } from '../../lib/controlledBetaCohortInterventionExecutionEnvelopeClient';
import { EnvelopeRecord, EnvelopeRuleCheck, EnvelopeEvidence, EnvelopeAuditLog } from '../../lib/controlledBetaCohortInterventionExecutionEnvelope';

const STATUS_COLORS: Record<string, string> = {
  DRAFT: '#6b7280',
  READY_FOR_EVALUATION: '#3b82f6',
  EVALUATED: '#10b981',
  READY_FOR_NO_OP_RUN: '#3b82f6',
  NO_OP_RUN_COMPLETED: '#10b981',
  FINALIZED: '#10b981',
  BLOCKED: '#dc2626',
  FAILED: '#ef4444',
  SUPERSEDED: '#9ca3af'
...
export const ControlledBetaCohortInterventionSimulationExecutionEnvelope: React.FC = () => {
  const [envelopeList, setEnvelopeList] = useState<EnvelopeRecord[]>([]);
  ...
  const [rules, setRules] = useState<EnvelopeRuleCheck[]>([]);
  const [evidence, setEvidence] = useState<EnvelopeEvidence | null>(null);
  const [auditLogs, setAuditLogs] = useState<EnvelopeAuditLog[]>([]);

  // Input states
  const [authIdInput, setAuthIdInput] = useState('');
  const [decisionRationale, setDecisionRationale] = useState('');
  const [selectedResult, setSelectedResult] = useState('NO_OP_EXECUTED_NOT_MUTATED');
  ...
  const loadList = async () => {
    setLoading(true);
    try {
      const data = await controlledBetaCohortInterventionExecutionEnvelopeClient.getEnvelopeList();
      setEnvelopeList(data);
      addLog('Fetched NO_OP execution envelope validation logs.');
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const loadDetails = async (envelopeId: string) => {
    setLoading(true);
    setError(null);
    try {
      const data = await controlledBetaCohortInterventionExecutionEnvelopeClient.getEnvelopeDetails(envelopeId);
      setEnvelope(data.envelope);
      setRules(data.rules);
      setEvidence(data.evidence);
      setAuditLogs(data.auditLogs);
      setActiveEnvelopeId(envelopeId);
      addLog(`Details loaded for NO_OP envelope: ${envelopeId}`);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const createRecord = async () => {
    ...
    setError(null);
    try {
      const res = await controlledBetaCohortInterventionExecutionEnvelopeClient.createEnvelope(authIdInput.trim());
      addLog(`Envelope draft ${res.envelope_id} initialized.`);
      setAuthIdInput('');
      await loadList();
      await loadDetails(res.envelope_id);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const evaluateRecord = async () => {
    ...
    try {
      ...
      if (overrideEnvelope === 'false') {
        overrides.canary_envelope = { mode: 'MUTATE_COHORTS', max_cohorts: 5 }; // triggers failure block
      }
      if (overrideResult) overrides.envelope_result = overrideResult;

      await controlledBetaCohortInterventionExecutionEnvelopeClient.evaluateEnvelope(envelope.envelope_id, overrides);
      addLog(`Completed rules validation for NO_OP envelope: ${envelope.envelope_id}`);
      await loadDetails(envelope.envelope_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  const recordDecision = async () => {
    if (!envelope || !decisionRationale.trim()) {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionEnvelopeClient.recordDecision(envelope.envelope_id, selectedResult, decisionRationale.trim());
  ...
  const finalizeRecord = async () => {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionEnvelopeClient.finalizeEnvelope(envelope.envelope_id);
      addLog(`Finalized NO_OP envelope packet. Evidence Pack v147.0 locked.`);
      await loadDetails(envelope.envelope_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Inter, system-ui, sans-serif', color: '#1e293b', background: '#f8fafc', minHeight: '100vh' }}>
      <header style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px', fontWeight: 700, letterSpacing: '-0.025em', color: '#0f172a' }}>
            High-Risk NO-OP Execution Envelope Gate
          </h1>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '14px' }}>
            Phase 147 Gate — Zero-mutation safety envelope validation, rate-limiting budgets and kill-switch overrides
          </p>
        </div>
      </header>
      ...
      <div style={{ background: '#fef3c7', borderLeft: '4px solid #d97706', padding: '16px', borderRadius: '4px', marginBottom: '24px' }}>
        <h4 style={{ margin: '0 0 4px 0', color: '#92400e', fontWeight: 600 }}>⚠️ WARNING: SAFE WORKFLOW BOUNDARIES PRESERVED (NO REAL MUTATIONS)</h4>
        <p style={{ margin: 0, color: '#b45309', fontSize: '13px' }}>
          This interface verifies the execution envelope parameters in **NO_OP mode only**. Zero active execution pathways exist. No database records inside user, session, or queue tables can be mutated.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px' }}>
        ...
        <div>
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>Initialize NO_OP Envelope</h3>
          ...
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>NO_OP Envelopes</h3>
            {loading && envelopeList.length === 0 ? (
              <p style={{ fontSize: '13px', color: '#64748b' }}>Loading...</p>
            ) : envelopeList.length === 0 ? (
              <p style={{ fontSize: '13px', color: '#64748b' }}>No envelopes found.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {envelopeList.map((env) => (
                  <div
                    key={env.envelope_id}
                    onClick={() => loadDetails(env.envelope_id)}
                    style={{
                      padding: '12px',
                      borderRadius: '6px',
                      border: '1px solid',
                      borderColor: activeEnvelopeId === env.envelope_id ? '#3b82f6' : '#e2e8f0',
                      background: activeEnvelopeId === env.envelope_id ? '#eff6ff' : '#f8fafc',
                      cursor: 'pointer',
                      transition: 'all 0.2s'
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 600, color: '#334155' }}>{env.envelope_id}</span>
                      <span style={{
                        fontSize: '10px',
                        fontWeight: 700,
                        padding: '2px 6px',
                        borderRadius: '4px',
                        background: STATUS_COLORS[env.envelope_status] || '#cbd5e1',
                        color: '#ffffff'
                      }}>{env.envelope_status}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Auth ID: {env.source_auth_id}</div>
                    {env.envelope_result && (
        ...
        <div>
          {envelope ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                  <div>
                    <h2 style={{ margin: 0, fontSize: '20px', color: '#0f172a' }}>NO_OP Envelope: {envelope.envelope_id}</h2>
                    <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#64748b' }}>
                      Derived from Authorization: <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{envelope.source_auth_id}</span>
                    </p>
                  </div>
                  <span style={{ padding: '6px 12px', borderRadius: '6px', background: '#f1f5f9', fontSize: '12px', fontWeight: 600 }}>
                    Status: {envelope.envelope_execution_status}
                  </span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>NO_OP Mode</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#10b981', marginTop: '2px' }}>
                      ENFORCED
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Runtime Mutations</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      0 (ZERO)
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Job Dispatch</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      {envelope.job_dispatch_status}
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Envelope Result</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#3b82f6', marginTop: '2px' }}>{envelope.envelope_result || 'PENDING EVALUATION'}</div>
                  </div>
                </div>

                {/* Decision Submit */}
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
                  <h4 style={{ margin: '0 0 12px 0', fontSize: '14px', fontWeight: 600 }}>Log Envelope Result</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '12px', marginBottom: '12px' }}>
                    ...
                    >
                      <option value="NO_OP_EXECUTED_NOT_MUTATED">NO_OP Executed (Not Mutated)</option>
                      <option value="NO_OP_BLOCKED_BY_GUARDRAIL">Blocked by Guardrail</option>
                      <option value="NO_OP_BLOCKED_BY_KILL_SWITCH">Blocked by Kill-Switch</option>
                      <option value="NO_OP_BLOCKED_BY_CANARY_LIMITS">Blocked by Canary Limits</option>
                      <option value="NO_OP_BLOCKED_BY_AUTHORIZATION_STATE">Blocked by Authorization State</option>
                      <option value="NO_OP_BLOCKED_BY_WRITE_SCOPE">Blocked by Write Scope</option>
                      <option value="REQUIRE_RE_AUTHORIZATION">Require Re-Authorization</option>
                  ...
                  <button
                    ...
                    disabled={envelope.envelope_status === 'FINALIZED'}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <button
                      ...
                      style={{ padding: '8px 16px', background: '#3b82f6', color: '#ffffff', border: 'none', borderRadius: '4px', fontSize: '13px', cursor: 'pointer', fontWeight: 500 }}
                    >
                      Evaluate NO_OP Envelope
                    </button>
                    <button
                      onClick={finalizeRecord}
                      disabled={!envelope.envelope_result || envelope.envelope_status === 'FINALIZED'}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px', background: '#faf5ff', padding: '16px', borderRadius: '6px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                    ...
                    <div>
                      <label style={{ fontSize: '11px', color: '#6b21a8' }}>NO_OP Envelope (0)</label>
                      <select
                      ...
                      >
                        ...
                        <option value="false">Invalid (Mutations &gt;0)</option>
                      </select>
                    </div>
                    <div>
                      ...
                      >
                        ...
                        <option value="NO_OP_EXECUTED_NOT_MUTATED">Force Executed</option>
                        <option value="NO_OP_BLOCKED_BY_GUARDRAIL">Force Blocked</option>
                      </select>
                    </div>
                  </div>
                </div>
              ...
              {evidence && (
                <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: 600 }}>Evidence Package (v147.0)</h3>
            ...
            <div style={{ background: '#ffffff', padding: '48px', borderRadius: '8px', border: '1px solid #e2e8f0', textAlign: 'center', color: '#64748b' }}>
              Select a NO_OP execution envelope record or enter a finalized authorization ID to start.
            </div>
          )}
        </div>
      </div>
