...
import { controlledBetaCohortInterventionExecutionPlanActivationReadinessClient } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationReadinessClient';
import { ReadinessRecord, ReadinessRuleCheck, ReadinessEvidence, ReadinessAuditLog } from '../../lib/controlledBetaCohortInterventionExecutionPlanActivationReadiness';
...
const STATUS_COLORS: Record<string, string> = {
  ...
  'ACTIVATION_READY': '#10b981',
  'ACTIVATION_NOT_READY': '#dc2626',
  FINALIZED: '#10b981',
  BLOCKED: '#dc2626',
  FAILED: '#ef4444',
  SUPERSEDED: '#9ca3af'
...
export const ControlledBetaCohortInterventionSimulationExecutionPlanActivationReadiness: React.FC = () => {
  const [readinessList, setReadinessList] = useState<ReadinessRecord[]>([]);
  const [activeRdId, setActiveRdId] = useState<string | null>(null);

  // Detail states
  const [rd, setRd] = useState<ReadinessRecord | null>(null);
  const [rules, setRules] = useState<ReadinessRuleCheck[]>([]);
  ...
  const [selectedResult, setSelectedResult] = useState('ACTIVATION_READY_NOT_ACTIVE');
  ...
  const loadList = async () => {
    setLoading(true);
    try {
      const data = await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.getReadinessList();
      setReadinessList(data);
      addLog('Fetched activation readiness logs.');
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const loadDetails = async (id: string) => {
    ...
    setError(null);
    try {
      const data = await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.getReadinessDetails(id);
      setRd(data.readiness);
      setRules(data.rules);
      setEvidence(data.evidence);
      setAuditLogs(data.auditLogs);
      setActiveRdId(id);
      addLog(`Details loaded for readiness record: ${id}`);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const createRecord = async () => {
    ...
    setError(null);
    try {
      const res = await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.createReadiness(planIdInput.trim());
      addLog(`Activation readiness draft ${res.activation_rd_id} initialized.`);
      setPlanIdInput('');
      await loadList();
      await loadDetails(res.activation_rd_id);
    } catch (e: any) {
      setError(e.message);
    }
    setLoading(false);
  ...
  const evaluateRecord = async () => {
    ...
    try {
      ...
      if (overrideHash === 'false') {
        overrides.canary_envelope = { activation_mode: 'EXECUTABLE', allow_real_activation: true }; // triggers failure block
      }
      if (overrideResult) overrides.activation_readiness_result = overrideResult;

      await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.evaluateReadiness(rd.activation_rd_id, overrides);
      addLog(`Completed rules validation for readiness: ${rd.activation_rd_id}`);
      await loadDetails(rd.activation_rd_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  const recordDecision = async () => {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.recordDecision(rd.activation_rd_id, selectedResult, decisionRationale.trim());
  ...
  const finalizeRecord = async () => {
    ...
    setError(null);
    try {
      await controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.finalizeReadiness(rd.activation_rd_id);
      addLog(`Finalized readiness packet. Evidence Pack v150.0 locked.`);
      await loadDetails(rd.activation_rd_id);
      await loadList();
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Inter, system-ui, sans-serif', color: '#1e293b', background: '#f8fafc', minHeight: '100vh' }}>
      <header style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px', fontWeight: 700, letterSpacing: '-0.025em', color: '#0f172a' }}>
            Activation Readiness Verification Gate
          </h1>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '14px' }}>
            Phase 150 Gate — Evaluates plan activation readiness conditions while maintaining strict non-active boundaries
          </p>
        </div>
      </header>
      ...
      <div style={{ background: '#fef3c7', borderLeft: '4px solid #d97706', padding: '16px', borderRadius: '4px', marginBottom: '24px' }}>
        <h4 style={{ margin: '0 0 4px 0', color: '#92400e', fontWeight: 600 }}>⚠️ WARNING: ACTIVATION READINESS ONLY (NON-ACTIVE)</h4>
        <p style={{ margin: 0, color: '#b45309', fontSize: '13px' }}>
          Activation readiness does not activate the execution plan. Activation readiness does not make the plan executable. Activation readiness does not create jobs, dispatch queues, or mutate runtime state.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px' }}>
        ...
        <div>
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>Verify Plan Readiness</h3>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                type="text"
                placeholder="Finalized Plan ID..."
                value={planIdInput}
          ...
          <div style={{ background: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '15px', fontWeight: 600 }}>Readiness Records</h3>
            {loading && readinessList.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {readinessList.map((item) => (
                  <div
                    key={item.activation_rd_id}
                    onClick={() => loadDetails(item.activation_rd_id)}
                    style={{
                      padding: '12px',
                      borderRadius: '6px',
                      border: '1px solid',
                      borderColor: activeRdId === item.activation_rd_id ? '#3b82f6' : '#e2e8f0',
                      background: activeRdId === item.activation_rd_id ? '#eff6ff' : '#f8fafc',
                      cursor: 'pointer',
                      transition: 'all 0.2s'
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 600, color: '#334155' }}>{item.activation_rd_id}</span>
                      <span style={{
                        fontSize: '10px',
                        fontWeight: 700,
                        padding: '2px 6px',
                        borderRadius: '4px',
                        background: STATUS_COLORS[item.activation_readiness_status] || '#cbd5e1',
                        color: '#ffffff'
                      }}>{item.activation_readiness_status}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Plan ID: {item.source_plan_id}</div>
                    {item.activation_readiness_result && (
        ...
        <div>
          {rd ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                  <div>
                    <h2 style={{ margin: 0, fontSize: '20px', color: '#0f172a' }}>Readiness Verification: {rd.activation_rd_id}</h2>
                    <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#64748b' }}>
                      Derived from Plan: <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{rd.source_plan_id}</span>
                    </p>
                  </div>
                  <span style={{ padding: '6px 12px', borderRadius: '6px', background: '#f1f5f9', fontSize: '12px', fontWeight: 600 }}>
                    Status: {rd.activation_execution_status}
                  </span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
                  ...
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Readiness Mode</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      {rd.activation_readiness_status}
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Real Jobs</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginTop: '2px' }}>
                      {rd.job_creation_status}
                    </div>
                  </div>
                  <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '6px' }}>
                    <div style={{ fontSize: '11px', color: '#64748b' }}>Readiness Result</div>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: '#3b82f6', marginTop: '2px' }}>{rd.activation_readiness_result || 'PENDING EVALUATION'}</div>
                  </div>
                </div>

                {/* Decision Submit */}
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
                  <h4 style={{ margin: '0 0 12px 0', fontSize: '14px', fontWeight: 600 }}>Log Activation Readiness Result</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '290px 1fr', gap: '12px', marginBottom: '12px' }}>
                    ...
                    >
                      <option value="ACTIVATION_READY_NOT_ACTIVE">Activation Ready (Not Active)</option>
                      <option value="ACTIVATION_BLOCKED_BY_PARENT_PLAN">Blocked by Parent Plan</option>
                      <option value="ACTIVATION_BLOCKED_BY_GUARDRAIL">Blocked by Guardrail</option>
                      <option value="ACTIVATION_BLOCKED_BY_EXECUTABLE_FLAG">Blocked by Executable Flag</option>
                      <option value="ACTIVATION_BLOCKED_BY_WRITE_SCOPE">Blocked by Write Scope</option>
                      <option value="ACTIVATION_BLOCKED_BY_MISSING_KILL_SWITCH">Blocked by Missing Kill-Switch</option>
                      <option value="ACTIVATION_BLOCKED_BY_MISSING_ROLLBACK_AUTHORITY">Blocked by Missing Rollback</option>
                      <option value="REQUIRE_PLAN_REMATERIALIZATION">Require Plan Rematerialization</option>
                  ...
                  <button
                    ...
                    disabled={rd.activation_readiness_status === 'FINALIZED'}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    <button
                      ...
                      disabled={!rd.activation_readiness_result || rd.activation_readiness_status === 'FINALIZED'}
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '20px', marginTop: '20px', background: '#faf5ff', padding: '16px', borderRadius: '6px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                    ...
                    <div>
                      <label style={{ fontSize: '11px', color: '#6b21a8' }}>Plan Hash Matching</label>
                      <select
                        value={overrideHash}
                        onChange={(e) => setOverrideHash(e.target.value)}
                        style={{ width: '100%', padding: '6px', fontSize: '12px', marginTop: '4px' }}
                      >
                        <option value="true">Matches (Pass)</option>
                        <option value="false">Mismatch (Fail)</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div style={{ marginTop: '12px' }}>
                    ...
                    >
                      ...
                      <option value="ACTIVATION_READY_NOT_ACTIVE">Force Ready</option>
                      <option value="ACTIVATION_BLOCKED_BY_GUARDRAIL">Force Blocked</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              {evidence && (
                <div style={{ background: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', fontWeight: 600 }}>Evidence Package (v150.0)</h3>
            ...
            <div style={{ background: '#ffffff', padding: '48px', borderRadius: '8px', border: '1px solid #e2e8f0', textAlign: 'center', color: '#64748b' }}>
              Select a readiness record or enter a finalized execution plan ID to start.
            </div>
          )}
        </div>
      </div>
