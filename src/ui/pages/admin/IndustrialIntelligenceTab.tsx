    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
                ...
                <div className="absolute bottom-0 left-1/2 w-96 h-96 bg-purple-600/10 blur-[120px] rounded-full" />
            </div>
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {(predictions.data?.predictions || []).map((p: any) => (
                            <div key={p.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between group hover:border-red-200 transition-all">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full border-4 border-white shadow-sm flex items-center justify-center bg-white overflow-hidden relative">
                                        <div className="absolute inset-0 bg-red-500/10" />
                                        <span className="text-[10px] font-black text-red-600 relative z-10">{Math.round(p.failure_probability * 100)}%</span>
                                    </div>
                                    <div>
                                        <p className="text-xs font-black text-slate-800 uppercase tracking-tight">Dispatch #{p.dispatch_id.slice(0, 8)}</p>
                                ...
                                <div className="text-right">
                                    ...
                                    <button className="px-3 py-1 bg-white border border-slate-200 text-[10px] font-black text-blue-600 rounded-lg hover:bg-blue-50 transition-all">AUTO-REROUTE</button>
                                </div>
                            </div>
                        ))}
                ...
                <div className="bg-slate-50 rounded-[2rem] border border-slate-200 p-6 shadow-sm">
                    ...
                    <div className="space-y-3">
                        {(congestion.data?.forecasts || []).slice(0, 6).map((f: any) => (
                            <div key={f.node_id} className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center justify-between">
                                <div className="flex-1">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-[10px] font-black text-slate-900 uppercase">Node #{f.node_id.slice(0, 8)}</span>
                                        <span className="text-[10px] font-black text-slate-400">{f.predicted_utilization_pct}% PROJECTED</span>
                                    </div>
                                    <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                                        <div 
                                            className={`h-full rounded-full transition-all duration-1000 ${f.predicted_utilization_pct > 80 ? 'bg-red-500' : 'bg-emerald-500'}`}
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {(reliability.data?.ranking || []).map((r: any) => (
                                    <tr key={r.printer_id} className="hover:bg-slate-50/50 transition-all">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-xs font-black text-slate-800">{r.company_name}</span>
                                                <span className="text-[10px] text-slate-400 font-bold uppercase">{r.city}, {r.country}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-3 py-1 rounded-full text-xs font-black ${
                                                r.trust_score > 90 ? 'bg-emerald-50 text-emerald-600' :
                                                r.trust_score > 70 ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'
                                            }`}>
                                                {r.trust_score}%
                                            </span>
                                        </td>
                ...
                <div className="bg-slate-900 rounded-[2rem] p-6 text-white shadow-xl">
                    ...
                    <div className="space-y-6">
                        {(federation.data?.snapshots || []).map((s: any) => (
                            <div key={s.region} className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                                ...
                                <div className="flex items-center gap-1 h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${s.resilience_score}%` }} />
                                </div>
                ...
                <div className="bg-white rounded-[2rem] border border-slate-200 p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {(optimization.data?.cycles || []).slice(0, 5).map((c: any) => (
                            <div key={c.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center gap-4">
                                <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center">
                                    <CpuChipIcon className="w-5 h-5 text-blue-600" />
                                </div>
                                <div>
                                    <p className="text-xs font-black text-slate-800 uppercase">{c.cycle_type}</p>
