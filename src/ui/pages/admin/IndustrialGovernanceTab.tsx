...
export const IndustrialGovernanceTab: React.FC = () => {
    ...
    return (
        <div className="space-y-8 pb-20">
            {/* Governance Header */}
            <div className="p-10 bg-indigo-950 rounded-[3rem] text-white shadow-2xl relative overflow-hidden border border-indigo-900">
                    ...
                    <div className="flex items-end justify-between gap-10">
                        ...
                        <div className="text-right">
                            <button 
                                ...
                                className="px-6 py-3 bg-indigo-500 hover:bg-indigo-400 text-white text-xs font-black uppercase tracking-widest rounded-2xl transition-all shadow-lg"
                            >
                ...
                <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-600/10 blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2" />
            </div>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {(resilience.data?.resilience || []).slice(0, 6).map((r: any) => (
                                <div key={r.id} className="p-5 bg-white/5 border border-white/10 rounded-2xl">
                                    ...
                                    <div className="flex items-end justify-between">
                                        ...
                                        <div className={`w-2 h-2 rounded-full ${r.survivability_score > 80 ? 'bg-indigo-400' : 'bg-orange-400'}`} />
                                    </div>
                    ...
                    <div className="space-y-4">
                        {(overview.data?.snapshots || []).slice(0, 5).map((s: any) => (
                            <div key={s.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <div className="text-right">
                                    <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-2 py-1 rounded-md">{s.governance_status}</span>
                                </div>
                            </div>
                        ))}
                    </div>
