...
export const ForensicsTab: React.FC = () => {
    ...
    return (
        <div className="space-y-6 animate-slide-fade">
            {/* Search Interface */}
            <div className="glass rounded-none border border-white p-6 shadow-none">
                <form onSubmit={handleSearch} className="flex gap-4">
                    <div className="flex-1 relative">
                        ...
                        <input 
                            ...
                            className="w-full pl-12 pr-4 py-3 rounded-none border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all font-mono text-sm"
                            value={jobId}
                    ...
                    <button 
                        ...
                        className="px-8 py-3 bg-slate-900 text-white font-bold rounded-none hover:bg-slate-800 disabled:opacity-50 transition-all flex items-center gap-2"
                    >
            ...
            {error && (
                <div className="p-6 rounded-none bg-red-50 border border-red-100 flex items-center gap-4 text-red-700">
            ...
            {data && (
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="glass rounded-none border border-white p-6 shadow-none space-y-4">
                            ...
                            <div className="pt-4 border-t border-slate-100">
                                ...
                                <div className="flex items-center gap-2 px-3 py-2 rounded-none bg-emerald-50 border border-emerald-100">
                            ...
                            <div className="relative space-y-8 before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                                {data.timeline.map((event: any, idx: number) => (
                                    <div key={idx} className="relative pl-12">
                                        <div className="absolute left-0 top-1 w-10 h-10 rounded-none bg-white border border-slate-200 flex items-center justify-center z-10">
                                            <div className={`w-3 h-3 rounded-none ${event.event.includes('FAIL') || event.event.includes('ERROR') ? 'bg-red-500' : 'bg-blue-500'}`} />
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            {event.metadata && Object.keys(event.metadata).length > 0 && (
                                                <div className="mt-2 p-3 rounded-none bg-slate-50 border border-slate-100 text-[11px] font-mono text-slate-600 overflow-x-auto whitespace-pre">
...
const DetailRow = ({ label, value, mono, highlight }: { label: string; value: string; mono?: boolean; highlight?: boolean }) => (
    <div>
        ...
        <div className={`text-xs mt-0.5 ${mono ? 'font-mono' : 'font-black'} ${highlight ? 'text-blue-600 bg-blue-50 px-2 py-0.5 rounded-none inline-block' : 'text-slate-800'}`}>
            {value}
        </div>
    </div>
);
