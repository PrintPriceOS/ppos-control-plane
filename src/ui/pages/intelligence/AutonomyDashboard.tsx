    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
                ...
                <Link to="/intelligence/autonomy/policies" className="px-4 py-2 bg-slate-900 text-white rounded-none font-medium text-sm hover:bg-slate-800">
                    Policy Framework
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                {strategies.map((s, idx) => (
                    <div key={idx} className={`bg-white border-2 rounded-none p-5 shadow-sm ${s.currentState === 'BOUNDED_AUTO' ? 'border-indigo-400' : 'border-slate-200'}`}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-slate-800">{s.strategyType}</h3>
                            <span className={`px-3 py-1 text-xs font-bold tracking-wider rounded-none ${
                        ...
                        <div className="pt-3 border-t text-sm">
                            ...
                            {s.isEligible ? (
                                <span className="text-green-600 flex items-center gap-2"><div className="w-2 h-2 rounded-none bg-green-500" /> Fully Eligible for Autonomy</span>
                            ) : (
                                <span className="text-amber-600 flex items-center gap-2"><div className="w-2 h-2 rounded-none bg-amber-500" /> {s.reason}</span>
                            )}
                        </div>
                    </div>
                ))}
