...
export const OptimizationPolicies: React.FC = () => {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-white border rounded-none p-6 shadow-sm">
                <h2 className="font-bold text-lg text-slate-800 mb-4">Execution Mode</h2>
                <div className="space-y-4">
                    <label className={`flex items-start p-4 border rounded-none cursor-pointer transition ${mode === 'SHADOW' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'hover:bg-slate-50'}`}>
                    ...
                    <label className={`flex items-start p-4 border rounded-none cursor-pointer transition ${mode === 'ADVISORY' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'hover:bg-slate-50'}`}>
                    ...
                    <label className={`flex items-start p-4 border rounded-none cursor-pointer transition opacity-50 ${mode === 'BOUNDED_AUTO' ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : ''}`}>
                        <input type="radio" className="mt-1" disabled checked={mode === 'BOUNDED_AUTO'} onChange={() => setMode('BOUNDED_AUTO')} />
                        <div className="ml-3">
                            <span className="block font-medium text-slate-800">BOUNDED AUTO <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-none ml-2">Phase 11 Locked</span></span>
                ...
                <div className="mt-6 flex justify-end">
                    <button className="px-4 py-2 bg-blue-600 text-white rounded-none text-sm font-medium hover:bg-blue-700 transition">
                        Save Policy
                    </button>
                </div>
            </div>
            
            <div className="bg-red-50 border border-red-100 rounded-none p-6 mt-8">
                 ...
                 <button className="px-5 py-2.5 bg-red-600 text-white font-bold rounded-none shadow hover:bg-red-700 transition uppercase tracking-wide text-sm">
