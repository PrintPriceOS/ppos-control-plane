...
export const AutonomyPolicies: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPolicy = async () => {
            try {
                const d = await adminFetch<any>('/api/admin/autonomy/policy');
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-none p-6 text-white text-sm font-mono shadow-md">
                <div className="flex items-center gap-3 mb-4">
                    <div className={`w-3 h-3 rounded-none ${policy.globalAutonomyEnabled ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></div>
                ...
                <pre className="bg-slate-950 p-4 rounded-none overflow-auto border border-slate-800 text-slate-300">
                    {JSON.stringify(policy.perTenantOverrides, null, 2)}
                </pre>
            </div>

            <div className="bg-amber-50 rounded-none p-5 border border-amber-200">
