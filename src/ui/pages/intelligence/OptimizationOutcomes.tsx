    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="space-y-4">
                {outcomes.length === 0 ? (
                    <div className="p-8 text-center text-slate-400 border-2 border-dashed rounded-none">No outcomes recorded yet. Apply a candidate.</div>
                ) : outcomes.map((o, idx) => (
                    <div key={idx} className="bg-white border rounded-none p-5 shadow-sm">
                        <div className="flex items-center justify-between">
                            ...
                            <div>
                                <span className={`px-3 py-1 rounded-none text-xs font-bold tracking-wide ${o.verdict === 'IMPROVED' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                    {o.verdict}
                                </span>
                            </div>
                        </div>
