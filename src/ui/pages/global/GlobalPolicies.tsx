    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 mt-6">
                {policies.length === 0 ? (
                    <div className="text-center p-8 text-slate-400 border-2 border-dashed rounded-none">No Global Policies registered.</div>
                ) : policies.map((p, i) => (
                    <div key={i} className={`bg-white rounded-none shadow-sm border p-5 ${p.status === 'ACTIVE' ? 'border-emerald-300 ring-1 ring-emerald-50' : 'border-slate-200'}`}>
                        <div className="flex flex-col md:flex-row justify-between md:items-center mb-3">
                            <div>
                                <h3 className="font-bold text-slate-800 text-lg flex items-center gap-3">
                                    {p.policyId}
                                    <span className="text-xs bg-slate-100 text-slate-500 font-mono px-2 py-1 rounded-none">v{p.version}</span>
                                </h3>
                                <div className="text-xs text-slate-400 tracking-wider uppercase mt-1">{p.directiveType} • Scope: {p.scope}</div>
                            </div>
                            <div className="mt-2 md:mt-0">
                                <span className={`text-[10px] font-bold px-3 py-1 rounded-none uppercase tracking-widest ${
