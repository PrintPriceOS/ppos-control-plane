    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div className="bg-white border rounded-none p-5 shadow-sm">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <span className={`px-2 py-0.5 rounded-none font-bold ${posture.degradedRegions.length > 0 ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>
                                {posture.degradedRegions.length}
                            </span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            {posture.degradedRegions.map((r: string) => (
                                <span key={r} className="text-xs bg-red-50 text-red-700 border border-red-100 px-2 py-1 rounded-none font-mono">{r}</span>
                            ))}
                ...
                <div className="bg-white border rounded-none p-5 shadow-sm">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-none font-bold">
                                {posture.autonomyRestrictedRegions.length}
                            </span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            {posture.autonomyRestrictedRegions.map((r: string) => (
                                <span key={r} className="text-xs bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-1 rounded-none font-mono">{r}</span>
                            ))}
            ...
            <div className="bg-slate-900 rounded-none p-6 text-white border border-slate-800 mt-6 font-mono text-xs">
