    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="space-y-4 mt-6">
                {rollouts.length === 0 ? (
                    <div className="text-center p-8 text-slate-400 border-2 border-dashed rounded-none">No active or historical rollouts found.</div>
                ) : rollouts.map((r, i) => (
                    <div key={i} className="bg-white rounded-none shadow-sm border border-slate-200 overflow-hidden">
                        <div className="bg-slate-50 p-4 border-b flex justify-between items-center">
                            ...
                            <span className={`text-[10px] font-bold px-2 py-1 rounded-none uppercase bg-indigo-100 text-indigo-700`}>{r.phase}</span>
                        </div>
                                ...
                                {r.blockedTargets.length === 0 && r.projectedConflicts.length === 0 ? (
                                    ...
                                    <div className="space-y-2">
                                        {[...r.blockedTargets, ...r.projectedConflicts].map((c: any, ci: number) => (
                                            <div key={ci} className="text-xs text-red-700 bg-red-50 p-2 rounded-none border border-red-100">
