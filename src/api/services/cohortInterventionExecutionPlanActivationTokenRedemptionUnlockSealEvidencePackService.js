...
class CohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealEvidencePackService {
  constructor() {
  ...
  async generateEvidencePack(record, actorId) {
    ...
    let parentLineage = {};
    if (!isProdLike) {
      const parent = frevBuilder._mockState.tokenRedemptionUnlockFinalReview.get(record.source_activation_token_redemption_unlock_final_review_id);
      if (parent) {
        parentLineage = parent.lineage_hash_chain_json || {};
      }
    } else {
      const rows = await db.query(
        `SELECT lineage_hash_chain_json FROM cb_cohort_intervention_activation_token_redempt_unlock_frev_ev WHERE activation_token_redemption_unlock_final_review_id = ?`,
    ...
    const lineageHashChain = {
      phase169_unlock_readiness_seal:
        record.unlock_seal_hash ||
        record.unlock_seal_evidence_pack_hash ||
        record.evidence_pack_hash ||
        'pending_hash',

      phase168_unlock_final_review:
        record.source_unlock_final_review_hash ||
        (parentLineage && (parentLineage.phase168_unlock_final_review || parentLineage.unlock_final_review_hash)) ||
    ...
    const payload = {
      evidence_schema_version: '169.0',
      activation_token_redemption_unlock_seal_id: record.activation_token_redemption_unlock_seal_id,
      source_activation_token_redemption_unlock_final_review_id: record.source_activation_token_redemption_unlock_final_review_id,
      ...
      unlock_seal_status: record.unlock_seal_status,
      unlock_seal_result: record.unlock_seal_result,
      lineageHashChain
    };
    ...
    if (!isProdLike) {
      ...
      this._mockEvidence.set(record.activation_token_redemption_unlock_seal_id, epRecord);
      return { evidence_pack_hash: evidencePackHash, lineageHashChain };
    }

    await db.query(
      `DELETE FROM cb_cohort_intervention_activation_token_redempt_unlock_seal_ev WHERE activation_token_redemption_unlock_seal_id = ?`,
      [record.activation_token_redemption_unlock_seal_id]
    );

    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_redempt_unlock_seal_ev
       (evidence_id, activation_token_redemption_unlock_seal_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [evidenceId, record.activation_token_redemption_unlock_seal_id, '169.0', evidencePackHash, JSON.stringify(payload), JSON.stringify(lineageHashChain)]
    );
...
module.exports = {
  serviceInstance: new CohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealEvidencePackService()
};
