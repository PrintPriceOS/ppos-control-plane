...
const tokenAuthEvidenceSvc = require('./cohortInterventionExecutionPlanActivationTokenAuthEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionPlanActivationTokenEnvEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(activationTokenEnvId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 155 evidence
    const parentEvidence = await tokenAuthEvidenceSvc.getEvidence(record.source_activation_token_auth_id);
    if (!parentEvidence) {
      throw new Error('PHASE155_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase155_activation_token_auth_id: record.source_activation_token_auth_id,
      phase155_source_activation_token_auth_hash: record.source_activation_token_auth_hash,
      phase155_source_activation_token_auth_evidence_pack_hash: parentEvidence.evidence_pack_hash,
    ...
    const rules = await builder.getRules(activationTokenEnvId);
    const rawPayload = {
      evidence_schema_version: '156.0',
      activation_token_env_id: record.activation_token_env_id,
      source_activation_token_auth_id: record.source_activation_token_auth_id,
      cohort_id: record.cohort_id,
      tenant_id: record.tenant_id,
      simulation_type: record.simulation_type,
      activation_token_env_status: record.activation_token_env_status,
      activation_token_env_result: record.activation_token_env_result,
    ...
    const evidencePackHash = 'evp_156_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_156_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(activationTokenEnvId, {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_env_evidence
       (evidence_id, activation_token_env_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '156.0', ?, ?, ?, ?)`,
      [
        evidenceId,
  ...
  async getEvidence(activationTokenEnvId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(activationTokenEnvId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_env_evidence WHERE activation_token_env_id = ?`,
      [activationTokenEnvId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationTokenEnvEvidencePackService();
module.exports = {
  CohortInterventionExecutionPlanActivationTokenEnvEvidencePackService,
  serviceInstance
};
