...
class CohortInterventionExecutionPlanActivationTokenRedemptionUnlockFinalHumanAuthorizationSealBuilderService {
  constructor() {
    this._mockState = {
      tokenRedemptionUnlockFinalHumanAuthorizationSeal: new Map(),
      rules: new Map()
    };
  }

  async createTokenRedemptionUnlockFinalHumanAuthorizationSealDraft(unlockDualControlAuthorizationId, actorId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    let parent = null;

    if (!isProdLike) {
      parent = dualControlBuilder._mockState.tokenRedemptionUnlockDualControlAuthorization.get(unlockDualControlAuthorizationId);
    } else {
    ...
    if (!parent) {
      throw new Error(`UNLOCK_DUAL_CONTROL_AUTHORIZATION_NOT_FOUND: Parent dual control record ${unlockDualControlAuthorizationId} does not exist.`);
    }

    if (parent.unlock_dual_control_authorization_status !== 'FINALIZED') {
      throw new Error(`UNLOCK_DUAL_CONTROL_AUTHORIZATION_NOT_READY: Parent dual control record ${unlockDualControlAuthorizationId} is not finalized.`);
    }

    if (parent.unlock_dual_control_authorization_result !== 'DUAL_CONTROL_AUTHORIZED_NOT_UNLOCKED') {
      throw new Error(`UNLOCK_DUAL_CONTROL_AUTHORIZATION_NOT_PASSED: Parent dual control record result is not DUAL_CONTROL_AUTHORIZED_NOT_UNLOCKED.`);
    }
    ...
    const unlockFinalHumanAuthorizationSealId = `fhas_${crypto.randomBytes(8).toString('hex')}`;
    const defaultCanary = {
      unlock_final_human_authorization_seal_mode: 'TOKEN_REDEMPTION_UNLOCK_FINAL_HUMAN_AUTHORIZATION_SEAL_GATE_ONLY',
      allow_unlock_final_human_authorization_seal_record: true,
    ...
    const record = {
      ...
      unlock_final_human_authorization_seal_result: 'PENDING',
      unlock_final_human_authorization_seal_mode: 'FINAL_HUMAN_AUTHORIZATION_SEAL_ONLY',
      unlock_dual_control_authorization_status: parent.unlock_dual_control_authorization_status || 'FINALIZED',
      ...
      unlock_final_human_authorization_seal_summary_json: { description: 'Unlock Final Human Authorization Seal Gate', state: 'DRAFT' },
      impact_review_json: { scope: 'FINAL_HUMAN_AUTHORIZATION_SEAL', impact: 'NONE' },
      ...
      non_execution_attestation_json: { attestation: 'NON_EXECUTABLE_FINAL_HUMAN_AUTHORIZATION_SEAL_ONLY' },
      write_scope_attestation_json: { scope: 'PHASE_173_TABLES_ONLY' },
      source_unlock_dual_control_authorization_hash: parent.unlock_dual_control_authorization_hash || 'dcau_hash_dummy',
      ...
      unlock_final_human_authorization_seal_hash: 'pending_hash',
      unlock_final_human_authorization_seal_evidence_pack_hash: 'pending_hash',
      evidence_pack_hash: 'pending_hash',
      lineage_hash_chain_json: {},
      security_signature_json: {},
      attestation_rationale_json: {},
      execution_capability_status: 'EXECUTION_NOT_ENABLED',
      activation_execution_status: 'UNLOCK_FINAL_HUMAN_AUTHORIZATION_SEAL_DRAFT_NOT_UNLOCKED_NOT_REDEEMED_NOT_EXECUTED',
      ...
      primary_authorizer_id: parent.primary_authorizer_id || 'dummy_alice',
      secondary_authorizer_id: parent.secondary_authorizer_id || 'dummy_bob',
      final_human_authorizer_id: null,
      final_human_authorizer_role: null,
      final_human_authorized_at: null,
      final_human_authorization_seal_reason: null,
      final_human_authorization_seal_attestation_json: {},
      dual_control_authorizer_separation_snapshot_json: {
        primary_authorizer_id: parent.primary_authorizer_id || 'dummy_alice',
        secondary_authorizer_id: parent.secondary_authorizer_id || 'dummy_bob'
      },
      created_by: actorId,
    ...
    if (!isProdLike) {
      this._mockState.tokenRedemptionUnlockFinalHumanAuthorizationSeal.set(unlockFinalHumanAuthorizationSealId, record);
      await auditService.logAction(unlockFinalHumanAuthorizationSealId, 'UNLOCK_FINAL_HUMAN_AUTHORIZATION_SEAL_DRAFT_CREATED', actorId);
      return { tokenRedemptionUnlockFinalHumanAuthorizationSeal: record };
    }
  ...
  async _internalUpdateUnlockFinalHumanAuthorizationSeal(unlockFinalHumanAuthorizationSealId, fields) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      const existing = this._mockState.tokenRedemptionUnlockFinalHumanAuthorizationSeal.get(unlockFinalHumanAuthorizationSealId);
      if (!existing) throw new Error(`Draft ${unlockFinalHumanAuthorizationSealId} not found.`);
      const updated = { ...existing, ...fields, updated_at: new Date() };
      this._mockState.tokenRedemptionUnlockFinalHumanAuthorizationSeal.set(unlockFinalHumanAuthorizationSealId, updated);
      return updated;
    }
    ...
    const jsonFields = [
      'canary_envelope_json', 'unlock_final_human_authorization_seal_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_final_human_authorization_seal_rules_json',
      'unlock_final_human_authorization_seal_blockers_json', 'non_execution_attestation_json',
      'write_scope_attestation_json', 'lineage_hash_chain_json', 'security_signature_json',
      'attestation_rationale_json', 'final_human_authorization_seal_attestation_json',
      'dual_control_authorizer_separation_snapshot_json'
    ];
  ...
  async getTokenRedemptionUnlockFinalHumanAuthorizationSeal(unlockFinalHumanAuthorizationSealId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
  ...
  async listTokenRedemptionUnlockFinalHumanAuthorizationSeals() {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return Array.from(this._mockState.tokenRedemptionUnlockFinalHumanAuthorizationSeal.values());
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_fhas ORDER BY created_at DESC`
    );
    const jsonFields = [
      'canary_envelope_json', 'unlock_final_human_authorization_seal_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_final_human_authorization_seal_rules_json',
...
module.exports = {
  serviceInstance: new CohortInterventionExecutionPlanActivationTokenRedemptionUnlockFinalHumanAuthorizationSealBuilderService()
};
