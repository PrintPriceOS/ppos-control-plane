...
const readinessEvidenceSvc = require('./cohortInterventionExecutionReadinessEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionAuthorizationEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(authId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 145 readiness evidence
    const parentEvidence = await readinessEvidenceSvc.getEvidence(record.source_readiness_id);
    if (!parentEvidence) {
      throw new Error('PHASE145_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase145_source_readiness_hash: record.source_readiness_hash,
      phase145_source_readiness_evidence_pack_hash: record.source_readiness_evidence_pack_hash,
    ...
    const rules = await builder.getRules(authId);
    const rawPayload = {
      evidence_schema_version: '146.0',
      auth_id: record.auth_id,
      source_readiness_id: record.source_readiness_id,
    ...
    const evidencePackHash = 'evp_146_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_146_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(authId, {
      ...
      await builder.updateAuth(authId, {
        auth_result_hash: evidencePackHash,
        evidence_pack_hash: evidencePackHash,
        lineage_hash_chain_json: lineageChain
      });
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_exec_auth_evidence
       (evidence_id, auth_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '146.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        authId,
  ...
  async getEvidence(authId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(authId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_exec_auth_evidence WHERE auth_id = ?`,
      [authId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionAuthorizationEvidencePackService();
module.exports = {
  CohortInterventionExecutionAuthorizationEvidencePackService,
  serviceInstance
};
