...
const SCAN_FILES = [
  'src/api/services/cohortInterventionExecutionPlanActivationHandoffBuilderService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationHandoffEvaluatorService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationHandoffEvidencePackService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationHandoffDecisionService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionPlanActivationHandoffAdmin.js'
];

class CohortInterventionExecutionPlanActivationHandoffGuardrailService {
  async performSafetyScannerCheck(activationHandoffId) {
    const findings = [];
    ...
    if (findings.length === 0) {
      findings.push({
        ...
        severity: 'INFO',
        description: 'Static scan of Phase 154 components confirms zero active activation handoff pathways or runtime table connections.'
      });
    }
  ...
  async verifyWriteScope(activationHandoffId) {
    ...
    const findings = [];
    if (!attestation || !attestation.writes_only_phase154_tables || attestation.wrote_phase128_to_153_operational_tables) {
      findings.push({
        check_type: 'WRITE_SCOPE_VIOLATION',
        severity: 'CRITICAL',
        description: 'Write scope validation failed. Phase 154 is strictly forbidden from mutating runtime tables.'
      });
    } else {
      findings.push({
        check_type: 'WRITE_SCOPE_VERIFICATION',
        severity: 'INFO',
        description: 'Verified write scope limits. Only Phase 154 schema structures are targeted.'
      });
    }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationHandoffGuardrailService();
module.exports = {
  CohortInterventionExecutionPlanActivationHandoffGuardrailService,
  serviceInstance
};
