...
const rdEvidenceSvc = require('./cohortInterventionExecutionPlanActivationReadinessEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionPlanActivationAuthorizationEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(activationAuthId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 150 readiness evidence
    const parentEvidence = await rdEvidenceSvc.getEvidence(record.source_activation_readiness_id);
    if (!parentEvidence) {
      throw new Error('PHASE150_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase150_source_activation_readiness_hash: record.source_activation_readiness_hash,
      phase150_source_activation_readiness_evidence_pack_hash: parentEvidence.evidence_pack_hash,
    ...
    const rules = await builder.getRules(activationAuthId);
    const rawPayload = {
      evidence_schema_version: '151.0',
      activation_auth_id: record.activation_auth_id,
      source_activation_readiness_id: record.source_activation_readiness_id,
      cohort_id: record.cohort_id,
      tenant_id: record.tenant_id,
      simulation_type: record.simulation_type,
      activation_auth_status: record.activation_auth_status,
      activation_auth_result: record.activation_auth_result,
    ...
    const evidencePackHash = 'evp_151_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_151_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(activationAuthId, {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_auth_evidence
       (evidence_id, activation_auth_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '151.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        activationAuthId,
  ...
  async getEvidence(activationAuthId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(activationAuthId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_auth_evidence WHERE activation_auth_id = ?`,
      [activationAuthId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationAuthorizationEvidencePackService();
module.exports = {
  CohortInterventionExecutionPlanActivationAuthorizationEvidencePackService,
  serviceInstance
};
