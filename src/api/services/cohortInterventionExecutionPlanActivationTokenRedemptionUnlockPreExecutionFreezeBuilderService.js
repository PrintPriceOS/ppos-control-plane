...
class CohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreezeBuilderService {
  constructor() {
    this._mockState = {
      tokenRedemptionUnlockPreExecutionFreeze: new Map(),
      rules: new Map()
    };
  }

  async createTokenRedemptionUnlockPreExecutionFreezeDraft(unlockSealId, actorId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    let parent = null;

    if (!isProdLike) {
      parent = sealBuilder._mockState.tokenRedemptionUnlockSeal.get(unlockSealId);
    } else {
    ...
    if (!parent) {
      throw new Error(`UNLOCK_SEAL_NOT_FOUND: Parent seal record ${unlockSealId} does not exist.`);
    }

    if (parent.unlock_seal_status !== 'FINALIZED') {
      throw new Error(`UNLOCK_SEAL_NOT_READY: Parent seal record ${unlockSealId} is not finalized.`);
    }

    if (parent.unlock_seal_result !== 'UNLOCK_READINESS_SEALED_NOT_UNLOCKED') {
      throw new Error(`UNLOCK_SEAL_NOT_PASSED: Parent seal record result is not UNLOCK_READINESS_SEALED_NOT_UNLOCKED.`);
    }
    ...
    const unlockPreExecutionFreezeId = `pfrz_${crypto.randomBytes(8).toString('hex')}`;
    const defaultCanary = {
      unlock_pre_execution_freeze_mode: 'TOKEN_REDEMPTION_UNLOCK_PRE_EXECUTION_FREEZE_GATE_ONLY',
      allow_unlock_pre_execution_freeze_record: true,
    ...
    const record = {
      ...
      unlock_pre_execution_freeze_result: 'PENDING',
      unlock_pre_execution_freeze_mode: 'PRE_EXECUTION_FREEZE_ONLY',
      ...
      unlock_pre_execution_freeze_summary_json: { description: 'Unlock Pre-Execution Freeze Gate', state: 'DRAFT' },
      impact_review_json: { scope: 'PRE_EXECUTION_FREEZE', impact: 'NONE' },
      ...
      non_execution_attestation_json: { attestation: 'NON_EXECUTABLE_PRE_EXECUTION_FREEZE_ONLY' },
      write_scope_attestation_json: { scope: 'PHASE_170_TABLES_ONLY' },
      source_unlock_seal_hash: parent.unlock_seal_hash || 'seal_hash_dummy',
      ...
      unlock_pre_execution_freeze_hash: 'pending_hash',
      unlock_pre_execution_freeze_evidence_pack_hash: 'pending_hash',
      evidence_pack_hash: 'pending_hash',
      lineage_hash_chain_json: {},
      security_signature_json: {},
      freeze_rationale_json: {},
      execution_capability_status: 'EXECUTION_NOT_ENABLED',
      activation_execution_status: 'UNLOCK_PRE_EXECUTION_FREEZE_DRAFT_NOT_UNLOCKED_NOT_REDEEMED_NOT_EXECUTED',
    ...
    if (!isProdLike) {
      this._mockState.tokenRedemptionUnlockPreExecutionFreeze.set(unlockPreExecutionFreezeId, record);
      await auditService.logAction(unlockPreExecutionFreezeId, 'UNLOCK_PRE_EXECUTION_FREEZE_DRAFT_CREATED', actorId);
      return { tokenRedemptionUnlockPreExecutionFreeze: record };
    }
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_redempt_unlock_pfrz
       (${columns.join(', ')})
       VALUES (${placeholders})`,
      values
    );
  ...
  async _internalUpdateUnlockPreExecutionFreeze(unlockPreExecutionFreezeId, fields) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      const existing = this._mockState.tokenRedemptionUnlockPreExecutionFreeze.get(unlockPreExecutionFreezeId);
      if (!existing) throw new Error(`Draft ${unlockPreExecutionFreezeId} not found.`);
      const updated = { ...existing, ...fields, updated_at: new Date() };
      this._mockState.tokenRedemptionUnlockPreExecutionFreeze.set(unlockPreExecutionFreezeId, updated);
      return updated;
    }
    ...
    await db.query(
      ...
      [...values, unlockPreExecutionFreezeId]
    );
    ...
    const jsonFields = [
      'canary_envelope_json', 'unlock_pre_execution_freeze_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_pre_execution_freeze_rules_json',
      'unlock_pre_execution_freeze_blockers_json', 'non_execution_attestation_json',
  ...
  async getTokenRedemptionUnlockPreExecutionFreeze(unlockPreExecutionFreezeId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return this._mockState.tokenRedemptionUnlockPreExecutionFreeze.get(unlockPreExecutionFreezeId);
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_pfrz WHERE activation_token_redemption_unlock_pre_execution_freeze_id = ?`,
      [unlockPreExecutionFreezeId]
  ...
  async listTokenRedemptionUnlockPreExecutionFreezes() {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return Array.from(this._mockState.tokenRedemptionUnlockPreExecutionFreeze.values());
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_pfrz ORDER BY created_at DESC`
    );
    const jsonFields = [
      'canary_envelope_json', 'unlock_pre_execution_freeze_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_pre_execution_freeze_rules_json',
  ...
  async getRules(unlockPreExecutionFreezeId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return this._mockState.rules.get(unlockPreExecutionFreezeId) || [];
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_pfrz_rl WHERE activation_token_redemption_unlock_pre_execution_freeze_id = ?`,
      [unlockPreExecutionFreezeId]
    );
    return rows;
  }
...
module.exports = {
  serviceInstance: new CohortInterventionExecutionPlanActivationTokenRedemptionUnlockPreExecutionFreezeBuilderService()
};
