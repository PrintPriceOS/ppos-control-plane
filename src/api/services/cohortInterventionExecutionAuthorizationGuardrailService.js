...
const SCAN_FILES = [
  'src/api/services/cohortInterventionExecutionAuthorizationBuilderService.js',
  'src/api/services/cohortInterventionExecutionAuthorizationEvaluatorService.js',
  'src/api/services/cohortInterventionExecutionAuthorizationEvidencePackService.js',
  'src/api/services/cohortInterventionExecutionAuthorizationDecisionService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionAuthorizationAdmin.js'
];

class CohortInterventionExecutionAuthorizationGuardrailService {
  async performSafetyScannerCheck(authId) {
    const findings = [];
    ...
    if (findings.length === 0) {
      findings.push({
        ...
        severity: 'INFO',
        description: 'Static scan of Phase 146 components confirms zero active execution capability pathways or runtime table connections.'
      });
    }
  ...
  async verifyWriteScope(authId) {
    ...
    const findings = [];
    if (!attestation || !attestation.writes_only_phase146_tables || attestation.wrote_phase128_to_145_operational_tables) {
      findings.push({
        check_type: 'WRITE_SCOPE_VIOLATION',
        severity: 'CRITICAL',
        description: 'Write scope validation failed. Phase 146 is strictly forbidden from mutating runtime tables.'
      });
    } else {
      findings.push({
        check_type: 'WRITE_SCOPE_VERIFICATION',
        severity: 'INFO',
        description: 'Verified write scope limits. Only Phase 146 schema structures are targeted.'
      });
    }
...
const serviceInstance = new CohortInterventionExecutionAuthorizationGuardrailService();
module.exports = {
  CohortInterventionExecutionAuthorizationGuardrailService,
  serviceInstance
};
