...
const envelopeEvidenceSvc = require('./cohortInterventionExecutionEnvelopeEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionDispatcherEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(dispatcherId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 147 envelope evidence
    const parentEvidence = await envelopeEvidenceSvc.getEvidence(record.source_envelope_id);
    if (!parentEvidence) {
      throw new Error('PHASE147_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase147_source_envelope_hash: record.source_envelope_hash,
      phase147_source_envelope_evidence_pack_hash: record.source_envelope_evidence_pack_hash,
    ...
    const rules = await builder.getRules(dispatcherId);
    const rawPayload = {
      evidence_schema_version: '148.0',
      ...
      dispatcher_status: record.dispatcher_status,
      dispatcher_result: record.dispatcher_result,
    ...
    const evidencePackHash = 'evp_148_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_148_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(dispatcherId, {
      ...
      await builder.updateDispatcher(dispatcherId, {
        dispatcher_result_hash: evidencePackHash,
        evidence_pack_hash: evidencePackHash,
        lineage_hash_chain_json: lineageChain
      });
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_dispatcher_evidence
       (evidence_id, dispatcher_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '148.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        dispatcherId,
  ...
  async getEvidence(dispatcherId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(dispatcherId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_dispatcher_evidence WHERE dispatcher_id = ?`,
      [dispatcherId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionDispatcherEvidencePackService();
module.exports = {
  CohortInterventionExecutionDispatcherEvidencePackService,
  serviceInstance
};
