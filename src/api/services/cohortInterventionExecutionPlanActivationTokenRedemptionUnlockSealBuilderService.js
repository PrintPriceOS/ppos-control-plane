...
class CohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealBuilderService {
  constructor() {
    this._mockState = {
      tokenRedemptionUnlockSeal: new Map(),
      rules: new Map()
    };
  }

  async createTokenRedemptionUnlockSealDraft(unlockFinalReviewId, actorId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    let parent = null;

    if (!isProdLike) {
      parent = frevBuilder._mockState.tokenRedemptionUnlockFinalReview.get(unlockFinalReviewId);
    } else {
    ...
    if (!parent) {
      throw new Error(`UNLOCK_FINAL_REVIEW_NOT_FOUND: Parent final review record ${unlockFinalReviewId} does not exist.`);
    }

    if (parent.unlock_final_review_status !== 'FINALIZED') {
      throw new Error(`UNLOCK_FINAL_REVIEW_NOT_READY: Parent final review record ${unlockFinalReviewId} is not finalized.`);
    }

    if (parent.unlock_final_review_result !== 'FINAL_REVIEW_PASSED_NOT_UNLOCKED') {
      throw new Error(`UNLOCK_FINAL_REVIEW_NOT_PASSED: Parent final review record result is not FINAL_REVIEW_PASSED_NOT_UNLOCKED.`);
    }
    ...
    const unlockSealId = `seal_${crypto.randomBytes(8).toString('hex')}`;
    const defaultCanary = {
      unlock_seal_mode: 'TOKEN_REDEMPTION_UNLOCK_READINESS_SEAL_GATE_ONLY',
      allow_unlock_seal_record: true,
    ...
    const record = {
      ...
      source_activation_token_redemption_unlock_final_review_id: unlockFinalReviewId,
      ...
      unlock_seal_result: 'PENDING',
      unlock_seal_mode: 'READINESS_SEAL_ONLY',
      ...
      unlock_seal_summary_json: { description: 'Unlock Readiness Seal Gate', state: 'DRAFT' },
      impact_review_json: { scope: 'READINESS_SEAL', impact: 'NONE' },
      ...
      non_execution_attestation_json: { attestation: 'NON_EXECUTABLE_READINESS_SEAL_ONLY' },
      write_scope_attestation_json: { scope: 'PHASE_169_TABLES_ONLY' },
      source_unlock_final_review_hash: parent.unlock_final_review_hash || 'frev_hash_dummy',
      ...
      unlock_seal_hash: 'pending_hash',
      unlock_seal_evidence_pack_hash: 'pending_hash',
      evidence_pack_hash: 'pending_hash',
      lineage_hash_chain_json: {},
      security_signature_json: {},
      seal_rationale_json: {},
      execution_capability_status: 'EXECUTION_NOT_ENABLED',
      activation_execution_status: 'UNLOCK_READINESS_SEAL_DRAFT_NOT_UNLOCKED_NOT_REDEEMED_NOT_EXECUTED',
    ...
    if (!isProdLike) {
      this._mockState.tokenRedemptionUnlockSeal.set(unlockSealId, record);
      await auditService.logAction(unlockSealId, 'UNLOCK_READINESS_SEAL_DRAFT_CREATED', actorId);
      return { tokenRedemptionUnlockSeal: record };
    }
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_redempt_unlock_seal
       (${columns.join(', ')})
       VALUES (${placeholders})`,
      values
    );
  ...
  async _internalUpdateUnlockSeal(unlockSealId, fields) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      const existing = this._mockState.tokenRedemptionUnlockSeal.get(unlockSealId);
      if (!existing) throw new Error(`Draft ${unlockSealId} not found.`);
      const updated = { ...existing, ...fields, updated_at: new Date() };
      this._mockState.tokenRedemptionUnlockSeal.set(unlockSealId, updated);
      return updated;
    }
    ...
    const jsonFields = [
      'canary_envelope_json', 'unlock_seal_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_seal_rules_json',
      'unlock_seal_blockers_json', 'non_execution_attestation_json',
  ...
  async getTokenRedemptionUnlockSeal(unlockSealId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return this._mockState.tokenRedemptionUnlockSeal.get(unlockSealId);
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_seal WHERE activation_token_redemption_unlock_seal_id = ?`,
      [unlockSealId]
  ...
  async listTokenRedemptionUnlockSeals() {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return Array.from(this._mockState.tokenRedemptionUnlockSeal.values());
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_seal ORDER BY created_at DESC`
    );
    const jsonFields = [
      'canary_envelope_json', 'unlock_seal_summary_json', 'impact_review_json',
      'rollback_review_json', 'guardrail_review_json', 'unlock_seal_rules_json',
  ...
  async getRules(unlockSealId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';
    if (!isProdLike) {
      return this._mockState.rules.get(unlockSealId) || [];
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_seal_rl WHERE activation_token_redemption_unlock_seal_id = ?`,
      [unlockSealId]
    );
    return rows;
  }
...
module.exports = {
  serviceInstance: new CohortInterventionExecutionPlanActivationTokenRedemptionUnlockSealBuilderService()
};
