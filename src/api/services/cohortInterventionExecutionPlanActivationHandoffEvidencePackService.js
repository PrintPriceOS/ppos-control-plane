...
const decisionEvidenceSvc = require('./cohortInterventionExecutionPlanActivationDecisionEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionPlanActivationHandoffEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(activationHandoffId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 153 decision evidence
    const parentEvidence = await decisionEvidenceSvc.getEvidence(record.source_activation_decision_id);
    if (!parentEvidence) {
      throw new Error('PHASE153_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase153_activation_decision_id: record.source_activation_decision_id,
      phase153_source_activation_decision_hash: record.source_activation_decision_hash,
      phase153_source_activation_decision_evidence_pack_hash: parentEvidence.evidence_pack_hash,
    ...
    const rules = await builder.getRules(activationHandoffId);
    const rawPayload = {
      evidence_schema_version: '154.0',
      activation_handoff_id: record.activation_handoff_id,
      source_activation_decision_id: record.source_activation_decision_id,
      cohort_id: record.cohort_id,
      tenant_id: record.tenant_id,
      simulation_type: record.simulation_type,
      activation_handoff_status: record.activation_handoff_status,
      activation_handoff_result: record.activation_handoff_result,
    ...
    const evidencePackHash = 'evp_154_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_154_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(activationHandoffId, {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_handoff_evidence
       (evidence_id, activation_handoff_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '154.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        activationHandoffId,
  ...
  async getEvidence(activationHandoffId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(activationHandoffId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_handoff_evidence WHERE activation_handoff_id = ?`,
      [activationHandoffId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationHandoffEvidencePackService();
module.exports = {
  CohortInterventionExecutionPlanActivationHandoffEvidencePackService,
  serviceInstance
};
