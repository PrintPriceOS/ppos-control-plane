...
const authEvidenceSvc = require('./cohortInterventionExecutionAuthorizationEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionEnvelopeEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(envelopeId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 146 authorization evidence
    const parentEvidence = await authEvidenceSvc.getEvidence(record.source_auth_id);
    if (!parentEvidence) {
      throw new Error('PHASE146_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase146_source_auth_hash: record.source_auth_hash,
      phase146_source_auth_evidence_pack_hash: record.source_auth_evidence_pack_hash,
    ...
    const rules = await builder.getRules(envelopeId);
    const rawPayload = {
      evidence_schema_version: '147.0',
      envelope_id: record.envelope_id,
      source_auth_id: record.source_auth_id,
    ...
    const evidencePackHash = 'evp_147_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_147_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(envelopeId, {
      ...
      await builder.updateEnvelope(envelopeId, {
        envelope_result_hash: evidencePackHash,
        evidence_pack_hash: evidencePackHash,
        lineage_hash_chain_json: lineageChain
      });
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_envelope_evidence
       (evidence_id, envelope_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '147.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        envelopeId,
  ...
  async getEvidence(envelopeId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(envelopeId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_envelope_evidence WHERE envelope_id = ?`,
      [envelopeId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionEnvelopeEvidencePackService();
module.exports = {
  CohortInterventionExecutionEnvelopeEvidencePackService,
  serviceInstance
};
