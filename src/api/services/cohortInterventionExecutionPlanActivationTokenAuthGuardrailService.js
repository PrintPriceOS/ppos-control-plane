...
const SCAN_FILES = [
  'src/api/services/cohortInterventionExecutionPlanActivationTokenAuthBuilderService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationTokenAuthEvaluatorService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationTokenAuthEvidencePackService.js',
  'src/api/services/cohortInterventionExecutionPlanActivationTokenAuthDecisionService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionPlanActivationTokenAuthAdmin.js'
];

class CohortInterventionExecutionPlanActivationTokenAuthGuardrailService {
  async performSafetyScannerCheck(activationTokenAuthId) {
    const findings = [];
    ...
    if (findings.length === 0) {
      findings.push({
        ...
        severity: 'INFO',
        description: 'Static scan of Phase 155 components confirms zero active activation pathways or runtime table connections.'
      });
    }
  ...
  async verifyWriteScope(activationTokenAuthId) {
    ...
    const findings = [];
    if (!attestation || !attestation.writes_only_phase155_tables || attestation.wrote_phase128_to_154_operational_tables) {
      findings.push({
        check_type: 'WRITE_SCOPE_VIOLATION',
        severity: 'CRITICAL',
        description: 'Write scope validation failed. Phase 155 is strictly forbidden from mutating runtime tables.'
      });
    } else {
      findings.push({
        check_type: 'WRITE_SCOPE_VERIFICATION',
        severity: 'INFO',
        description: 'Verified write scope limits. Only Phase 155 schema structures are targeted.'
      });
    }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationTokenAuthGuardrailService();
module.exports = {
  CohortInterventionExecutionPlanActivationTokenAuthGuardrailService,
  serviceInstance
};
