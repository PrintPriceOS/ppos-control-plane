...
const SCAN_FILES = [
  'src/api/services/cohortInterventionExecutionPlanBuilderService.js',
  'src/api/services/cohortInterventionExecutionPlanEvaluatorService.js',
  'src/api/services/cohortInterventionExecutionPlanEvidencePackService.js',
  'src/api/services/cohortInterventionExecutionPlanDecisionService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionPlanAdmin.js'
];

class CohortInterventionExecutionPlanGuardrailService {
  async performSafetyScannerCheck(planId) {
    const findings = [];
    ...
    if (findings.length === 0) {
      findings.push({
        ...
        severity: 'INFO',
        description: 'Static scan of Phase 149 components confirms zero active execution plan materialization pathways or runtime table connections.'
      });
    }
  ...
  async verifyWriteScope(planId) {
    ...
    const findings = [];
    if (!attestation || !attestation.writes_only_phase149_tables || attestation.wrote_phase128_to_148_operational_tables) {
      findings.push({
        check_type: 'WRITE_SCOPE_VIOLATION',
        severity: 'CRITICAL',
        description: 'Write scope validation failed. Phase 149 is strictly forbidden from mutating runtime tables.'
      });
    } else {
      findings.push({
        check_type: 'WRITE_SCOPE_VERIFICATION',
        severity: 'INFO',
        description: 'Verified write scope limits. Only Phase 149 schema structures are targeted.'
      });
    }
...
const serviceInstance = new CohortInterventionExecutionPlanGuardrailService();
module.exports = {
  CohortInterventionExecutionPlanGuardrailService,
  serviceInstance
};
