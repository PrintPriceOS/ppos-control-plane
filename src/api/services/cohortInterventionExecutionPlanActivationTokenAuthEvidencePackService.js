...
const handoffEvidenceSvc = require('./cohortInterventionExecutionPlanActivationHandoffEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionPlanActivationTokenAuthEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(activationTokenAuthId, actorId = 'system') {
    ...
    // 1. Fetch parent Phase 154 handoff evidence
    const parentEvidence = await handoffEvidenceSvc.getEvidence(record.source_activation_handoff_id);
    if (!parentEvidence) {
      throw new Error('PHASE154_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase154_activation_handoff_id: record.source_activation_handoff_id,
      phase154_source_activation_handoff_hash: record.source_activation_handoff_hash,
      phase154_source_activation_handoff_evidence_pack_hash: parentEvidence.evidence_pack_hash,
      phase153_activation_decision_id: parentLineage.phase153_activation_decision_id || 'none',
      phase153_source_activation_decision_hash: record.source_activation_decision_hash,
    ...
    const rules = await builder.getRules(activationTokenAuthId);
    const rawPayload = {
      evidence_schema_version: '155.0',
      activation_token_auth_id: record.activation_token_auth_id,
      source_activation_handoff_id: record.source_activation_handoff_id,
      cohort_id: record.cohort_id,
      tenant_id: record.tenant_id,
      simulation_type: record.simulation_type,
      activation_token_auth_status: record.activation_token_auth_status,
      activation_token_auth_result: record.activation_token_auth_result,
    ...
    const evidencePackHash = 'evp_155_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_155_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(activationTokenAuthId, {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_auth_evidence
       (evidence_id, activation_token_auth_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '155.0', ?, ?, ?, ?)`,
      [
        evidenceId,
  ...
  async getEvidence(activationTokenAuthId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(activationTokenAuthId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_auth_evidence WHERE activation_token_auth_id = ?`,
      [activationTokenAuthId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionPlanActivationTokenAuthEvidencePackService();
module.exports = {
  CohortInterventionExecutionPlanActivationTokenAuthEvidencePackService,
  serviceInstance
};
