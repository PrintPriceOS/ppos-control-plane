...
const dispatcherEvidenceSvc = require('./cohortInterventionExecutionDispatcherEvidencePackService').serviceInstance;
...
class CohortInterventionExecutionPlanEvidencePackService {
  constructor() {
    this._mockState = {
      evidence: new Map()
    };
  ...
  async buildEvidencePack(planId, actorId = 'system') {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    const record = await builder.getPlan(planId);
    if (!record) throw new Error('PLAN_RECORD_NOT_FOUND');

    // 1. Fetch parent Phase 148 dispatcher evidence
    const parentEvidence = await dispatcherEvidenceSvc.getEvidence(record.source_dispatcher_id);
    if (!parentEvidence) {
      throw new Error('PHASE148_EVIDENCE_NOT_FOUND');
    }
    ...
    const lineageChain = {
      ...
      phase148_source_dispatcher_hash: record.source_dispatcher_hash,
      phase148_source_dispatcher_evidence_pack_hash: record.source_dispatcher_evidence_pack_hash,
    ...
    const rules = await builder.getRules(planId);
    const rawPayload = {
      evidence_schema_version: '149.0',
      plan_id: record.plan_id,
      source_dispatcher_id: record.source_dispatcher_id,
    ...
    const evidencePackHash = 'evp_149_' + crypto.createHash('sha256').update(payloadStr).digest('hex');

    const evidenceId = 'evd_149_' + crypto.randomBytes(8).toString('hex');
    const created = new Date();

    if (!isProdLike) {
      this._mockState.evidence.set(planId, {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_plan_evidence
       (evidence_id, plan_id, evidence_schema_version, evidence_pack_hash, evidence_payload_json, lineage_hash_chain_json, created_at)
       VALUES (?, ?, '149.0', ?, ?, ?, ?)`,
      [
        evidenceId,
        planId,
  ...
  async getEvidence(planId) {
    const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

    if (!isProdLike) {
      return this._mockState.evidence.get(planId) || null;
    }

    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_plan_evidence WHERE plan_id = ?`,
      [planId]
    );
    return rows[0] || null;
  }
...
const serviceInstance = new CohortInterventionExecutionPlanEvidencePackService();
module.exports = {
  CohortInterventionExecutionPlanEvidencePackService,
  serviceInstance
};
