...
class CohortInterventionExecutionPlanActivationTokenRedemptionUnlockFinalNonExecutionEvidenceSealBuilderService {
  constructor() {
    this._mockState = {
      tokenRedemptionUnlockFinalNonExecutionEvidenceSeal: new Map(),
      rules: new Map()
    };
  }

  async createTokenRedemptionUnlockFinalNonExecutionEvidenceSealDraft(unlockKillSwitchDryRunId, actorId) {
    const parent = await parentBuilder.getTokenRedemptionUnlockKillSwitchDryRun(unlockKillSwitchDryRunId);
    if (!parent) {
      throw new Error(`Parent emergency rollback authority record ${unlockKillSwitchDryRunId} not found.`);
    }

    if (parent.unlock_kill_switch_dry_run_status !== 'FINALIZED') {
      throw new Error(`Parent emergency rollback authority must be FINALIZED. Current status: ${parent.unlock_kill_switch_dry_run_status}`);
    }

    if (parent.unlock_kill_switch_dry_run_result !== 'KILL_SWITCH_DRY_RUN_VERIFIED_NOT_UNLOCKED') {
      throw new Error(`Parent emergency rollback authority result must be KILL_SWITCH_DRY_RUN_VERIFIED_NOT_UNLOCKED. Current result: ${parent.unlock_kill_switch_dry_run_result}`);
    }
    ...
    const draft = {
      ...
      source_unlock_kill_switch_dry_run_hash: parent.unlock_kill_switch_dry_run_hash || 'h1',
      ...
      unlock_final_non_execution_evidence_seal_hash: '',
      unlock_final_non_execution_evidence_seal_evidence_pack_hash: '',
      evidence_pack_hash: '',
      lineage_hash_chain_json: '{}',
      security_signature_json: '{}',
      attestation_rationale_json: '{}',
      execution_capability_status: 'EXECUTION_NOT_ENABLED',
      activation_execution_status: 'UNLOCK_FINAL_NON_EXECUTION_EVIDENCE_SEAL_DRAFT_NOT_UNLOCKED_NOT_REDEEMED_NOT_EXECUTED',
      ...
      evidence_sealed_at: null,
      evidence_seal_reason: null,
    ...
    await auditService.logAction(unlockFinalNonExecutionEvidenceSealId, 'UNLOCK_FINAL_NON_EXECUTION_EVIDENCE_SEAL_DRAFT_CREATED', actorId);

    return { tokenRedemptionUnlockFinalNonExecutionEvidenceSeal: draft };
  }

  async getTokenRedemptionUnlockFinalNonExecutionEvidenceSeal(id) {
    if (!isProdLike) {
      return this._mockState.tokenRedemptionUnlockFinalNonExecutionEvidenceSeal.get(id) || null;
    }
    const rows = await db.query(
      `SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_fnees
    ...
    r.final_non_execution_evidence_snapshot_json = parseJsonField(r.final_non_execution_evidence_snapshot_json, {});
    r.non_execution_evidence_snapshot_json = parseJsonField(r.non_execution_evidence_snapshot_json, {});
    r.prior_authorizer_separation_snapshot_json = parseJsonField(r.prior_authorizer_separation_snapshot_json, {});
    r.final_non_execution_evidence_rules_json = parseJsonField(r.final_non_execution_evidence_rules_json, {});
    return r;
  }

  async _internalUpdateTokenRedemptionUnlockFinalNonExecutionEvidenceSeal(id, updates) {
    if (!isProdLike) {
      const current = this._mockState.tokenRedemptionUnlockFinalNonExecutionEvidenceSeal.get(id);
  ...
  async listTokenRedemptionUnlockFinalNonExecutionEvidenceSeal() {
    if (!isProdLike) {
      return Array.from(this._mockState.tokenRedemptionUnlockFinalNonExecutionEvidenceSeal.values());
    }
    const rows = await db.query(`SELECT * FROM cb_cohort_intervention_activation_token_redempt_unlock_fnees ORDER BY created_at DESC`);
    return rows.map(r => {
      r.canary_envelope_json = parseJsonField(r.canary_envelope_json, {});
      r.impact_review_json = parseJsonField(r.impact_review_json, {});
      r.rollback_review_json = parseJsonField(r.rollback_review_json, {});
...
module.exports = {
  serviceInstance: new CohortInterventionExecutionPlanActivationTokenRedemptionUnlockFinalNonExecutionEvidenceSealBuilderService()
};
