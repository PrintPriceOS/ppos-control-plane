...
console.log('=== Phase 134 Invite Acceptance Acceptance Pack (134H) ===\n');
...
(async () => {
  ...
  const smokes = [
    'smoke_phase134a_invite_acceptance_schema.js',
    'smoke_phase134b_invite_acceptance_service.js',
    'smoke_phase134c_invite_acceptance_readiness.js',
    'smoke_phase134d_onboarding_scope_session_limits.js',
    'smoke_phase134e_runtime_access_guardrails.js',
    'smoke_phase134f_invite_acceptance_admin_api_ui.js',
    'smoke_phase134g_invite_acceptance_evidence_pack.js',
    // Run Phase 133H as dependency if DB is reachable
    ...(hasDb ? ['smoke_phase133h_invite_issuance_acceptance_pack.js'] : [])
  ];
  ...
  const svcPath = path.join(__dirname, '../src/api/services/controlledBetaInviteAcceptanceService.js');
  ...
  console.log(`\nSmoke 134H: Finished execution. ${passed} passed, ${failed} failed`);
  ...
  if (failed > 0 || failedSmokes.length > 0) {
    ...
    console.error('\nFATAL: Phase 134 Acceptance Pack Failed.');
    process.exit(1);
  } else {
    console.log('\nSUCCESS: Phase 134 Acceptance Pack Passed.');
    process.exit(0);
  }
})().catch(err => {
  console.error('FATAL error in 134H:', err);
  process.exit(1);
});
