...
const decision = require('../src/api/services/cohortInterventionExecutionDispatcherDecisionService').serviceInstance;
...
async function setupEnvelopeAndDispatcher(envelopeId, dispatcherId) {
  ...
  const envelopeRecord = {
    ...
    source_auth_id: 'ath_test_148d',
    source_readiness_id: 'rd_test_148d',
    source_approval_id: 'apv_test_148d',
    source_prep_id: 'prep_test_148d',
    source_review_id: 'rev_test_148d',
    source_simulation_id: 'sim_test_148d',
    source_execution_id: 'exec_test_148d',
    cohort_id: 'cohort_test_148d',
    tenant_id: 'tenant_test_148d',
    ...
    source_auth_hash: 'auth_hash_148d',
    source_auth_evidence_pack_hash: 'ae_hash_148d',
    envelope_result_hash: 'result_hash_148d',
    evidence_pack_hash: 'pack_hash_148d',
  ...
  const dispatcherRecord = {
    ...
    source_envelope_hash: 'result_hash_148d',
    source_envelope_evidence_pack_hash: 'pack_hash_148d',
  ...
  if (!isProdLike) {
    ...
    envelopeEvidenceSvc._mockState.evidence.set(envelopeId, {
      ...
      lineage_hash_chain_json: {
        ...
        phase143_preparation_id: 'prep_test_148d',
        phase142_review_id: 'rev_test_148d',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_no_op_envelope
       ...
       VALUES (?, 'ath_test_148d', 'rd_test_148d', 'apv_test_148d', 'prep_test_148d', 'rev_test_148d', 'sim_test_148d', 'exec_test_148d', 'cohort_test_148d', 'tenant_test_148d', 'SIMULATE_COHORT_PAUSE',
        'FINALIZED', 'NO_OP_EXECUTED_NOT_MUTATED', 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PASS', 'PASS', '{"mode":"NO_OP", "max_cohorts":0, "max_participants":0}', '{}', '{}', '{}', '{}', '{}', '{}', ?, ?, 'auth_hash_148d', 'ae_hash_148d', 'EXECUTION_NOT_ENABLED', 'NO_OP_ENVELOPE_ACTIVE_NOT_MUTATING', 'NO_OP_EXECUTED_NOT_MUTATED', 'ZERO_RUNTIME_MUTATION_CONFIRMED', 'NO_JOB_DISPATCHED', 'result_hash_148d', 'pack_hash_148d')`,
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_dry_run_dispatcher
       ...
       VALUES (?, ?, 'ath_test_148d', 'rd_test_148d', 'apv_test_148d', 'prep_test_148d', 'rev_test_148d', 'sim_test_148d', 'exec_test_148d', 'cohort_test_148d', 'tenant_test_148d', 'SIMULATE_COHORT_PAUSE',
        'DRAFT', NULL, 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PENDING', 'PENDING', '{"dispatch_mode":"DRY_RUN_ONLY", "queue_dispatch_mode":"SIMULATED_ONLY", "allow_real_job_creation":false, "max_runtime_mutations":0}', '{}', '{}', '{}', '{}', '{}', '{"missing_dispatcher_evaluation":true}', ?, ?, 'result_hash_148d', 'pack_hash_148d', 'EXECUTION_NOT_ENABLED', 'DRY_RUN_ACTIVE_NOT_MUTATING', 'DRY_RUN_EXECUTED_NOT_MUTATED', 'SIMULATED_ONLY', 'ZERO_RUNTIME_MUTATION_CONFIRMED', 'NO_REAL_JOB_CREATED')`,
...
(async () => {
  console.log('=== Smoke 148D: Review Workflow Governance ===\n');

  try {
    const envelopeId = 'env_148d_1';
    const dispatcherId = 'dsp_148d_1';
    await setupEnvelopeAndDispatcher(envelopeId, dispatcherId);

    // 1. Finalization blocks before evaluation
    try {
      await decision.finalizeDispatcher(dispatcherId, 'admin');
      assert.fail('Should block finalization when evaluation/decision are not done');
    } catch (e) {
      if (e.message.includes('DISPATCHER_EVALUATION_NOT_COMPLETED') || e.message.includes('DISPATCHER_DECISION_REQUIRED')) {
    ...
    // 3. Record decision
    await decision.recordDecision(dispatcherId, 'DRY_RUN_EXECUTED_NOT_MUTATED', 'Dry-run dispatcher verification complete.', 'admin');

    // 4. Build evidence
    await evidenceSvc.buildEvidencePack(dispatcherId, 'admin');

    // 5. Finalize
    const { dispatcher } = await decision.finalizeDispatcher(dispatcherId, 'admin');
    assert.strictEqual(dispatcher.dispatcher_status, 'FINALIZED');
    assert.strictEqual(dispatcher.execution_capability_status, 'EXECUTION_NOT_ENABLED');
    assert.strictEqual(dispatcher.dispatcher_execution_status, 'DRY_RUN_ACTIVE_NOT_MUTATING');
    assert.strictEqual(dispatcher.dry_run_execution_result, 'DRY_RUN_EXECUTED_NOT_MUTATED');
    assert.strictEqual(dispatcher.queue_dispatch_status, 'SIMULATED_ONLY');
    console.log('  PASS: Dispatcher package finalized successfully with safe non-execution markers.');

    // 6. Block modification after finalization
    try {
      await decision.recordDecision(dispatcherId, 'DRY_RUN_BLOCKED_BY_GUARDRAIL', 'Change mind', 'admin');
      assert.fail('Should block modifications on finalized dispatcher');
    } catch (e) {
      if (e.message.includes('DISPATCHER_RECORD_ALREADY_FINALIZED')) {
        console.log('  PASS: Modifications blocked on finalized dispatcher.');
      } else {
        throw e;
      }
    }
    ...
    console.error('FAIL in 148D:', e.message);
    process.exit(1);
  } finally {
