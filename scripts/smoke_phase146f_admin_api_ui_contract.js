...
const adminRouter = require('../src/api/routes/controlledBetaCohortInterventionExecutionAuthorizationAdmin');

(async () => {
  console.log('=== Smoke 146F: Admin API & UI Contract Verification ===\n');
  ...
  try {
    ...
    const expectedRoutes = [
      '/auth',
      '/auth/:authId',
      '/auth/from-readiness/:readinessId',
      '/auth/:authId/evaluate',
      '/auth/:authId/decision',
      '/auth/:authId/finalize'
    ];
    ...
    const clientPath = path.join(__dirname, '../src/ui/lib/controlledBetaCohortInterventionExecutionAuthorizationClient.ts');
    ...
    const pagePath = path.join(__dirname, '../src/ui/pages/beta/ControlledBetaCohortInterventionSimulationExecutionAuthorization.tsx');
    ...
    assert.ok(
      ...
      pageContent.includes('dry-run envelope verification') ||
    ...
    console.error('FAIL in 146F:', e.message);
    process.exit(1);
  }
})();
