...
const PHASE = '167';
const PHASE_NAME = 'CONTROLLED HIGH-RISK COHORT INTERVENTION ACTIVATION TOKEN REDEMPTION UNLOCK APPROVAL GATE';

const SMOKE_TESTS = [
  { name: `smoke_phase${PHASE}a_activation_token_redemption_unlock_approval_schema.js`, label: `Smoke ${PHASE}A: Phase ${PHASE} Schema Validation` },
  { name: `smoke_phase${PHASE}b_create_unlock_approval_from_phase166_eligibility.js`, label: `Smoke ${PHASE}B: Create Unlock Approval Draft` },
  { name: `smoke_phase${PHASE}c_activation_token_redemption_unlock_approval_evaluator_rules.js`, label: `Smoke ${PHASE}C: Activation Token Redemption Unlock Approval Evaluator Rules` },
  { name: `smoke_phase${PHASE}d_unlock_approval_workflow_governance.js`, label: `Smoke ${PHASE}D: Unlock Approval Workflow Governance` },
  ...
  { name: `smoke_phase${PHASE}g_guardrail_unlock_approval_not_unlocked_write_scope.js`, label: `Smoke ${PHASE}G: Guardrails & Safety Boundary Scanner` }
];
...
(async () => {
  ...
  console.log(`--- Validating Phase 166H Dependency ---`);
  try {
    const phase166HPath = path.join(__dirname, 'smoke_phase166h_acceptance_pack_real_db.js');
    const phase166HOutput = execFileSync(process.execPath, ['-r', 'dotenv/config', phase166HPath], {
      env: { ...process.env, ALLOW_SCHEMA_SMOKE_FALLBACK: 'true', ALLOW_SMOKE_FALLBACK: 'true' },
      stdio: 'pipe',
      cwd: path.join(__dirname, '..')
    });
    console.log(phase166HOutput.toString());
    console.log(`  PASS: Phase 166H dependency validated successfully.`);
  } catch (e) {
    console.warn(`  WARN: Phase 166H dependency check failed (non-blocking in mock mode): ${e.message}`);
  }
  ...
  console.log(`ACTIVATION EXECUTION STATUS: UNLOCK_APPROVAL_FINALIZED_NOT_UNLOCKED_NOT_REDEEMED_NOT_EXECUTED`);
