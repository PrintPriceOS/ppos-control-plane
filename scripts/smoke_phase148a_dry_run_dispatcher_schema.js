...
(async () => {
  console.log('=== Smoke 148A: Phase 148 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_dry_run_dispatcher',
      'cb_cohort_intervention_dispatcher_rules',
      'cb_cohort_intervention_dispatcher_evidence',
      'cb_cohort_intervention_dispatcher_audits'
    ];
    ...
    // Validate key columns on main dispatcher table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_dry_run_dispatcher'`
    );
    ...
    for (const col of [
      ...
      'dispatcher_status', 'dispatcher_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_dry_run_dispatcher.${col}' exists.`);
    }

    console.log('\nSmoke 148A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 148A:', e.message);
    process.exit(1);
  } finally {
