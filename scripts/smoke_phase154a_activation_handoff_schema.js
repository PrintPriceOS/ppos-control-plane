...
(async () => {
  console.log('=== Smoke 154A: Phase 154 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_handoff',
      'cb_cohort_intervention_activation_handoff_rules',
      'cb_cohort_intervention_activation_handoff_evidence',
      'cb_cohort_intervention_activation_handoff_audits'
    ];
    ...
    // Validate key columns on main activation handoff table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_handoff'`
    );
    ...
    for (const col of [
      ...
      'activation_handoff_status', 'activation_handoff_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_handoff.${col}' exists.`);
    }

    console.log('\nSmoke 154A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 154A:', e.message);
    process.exit(1);
  } finally {
