...
const builder = require('../src/api/services/cohortInterventionExecutionDispatcherBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionDispatcherGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionDispatcherAdmin.js'
];

(async () => {
  console.log('=== Smoke 148G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      // Run dummy dispatcher builder call
      try {
        await builder.createDispatcher('non_existent_envelope', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 148 operations!`);
    ...
    console.error('FAIL in 148G:', e.message);
    process.exit(1);
  } finally {
