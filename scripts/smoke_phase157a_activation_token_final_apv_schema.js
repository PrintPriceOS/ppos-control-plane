...
(async () => {
  console.log('=== Smoke 157A: Phase 157 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_token_final_apv',
      'cb_cohort_intervention_activation_token_final_apv_rules',
      'cb_cohort_intervention_activation_token_final_apv_evidence',
      'cb_cohort_intervention_activation_token_final_apv_audits'
    ];
    ...
    // Validate key columns on main activation token final apv table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_token_final_apv'`
    );
    ...
    for (const col of [
      ...
      'activation_token_final_apv_status', 'activation_token_final_apv_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_token_final_apv.${col}' exists.`);
    }

    console.log('\nSmoke 157A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 157A:', e.message);
    process.exit(1);
  } finally {
