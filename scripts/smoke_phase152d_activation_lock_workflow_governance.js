...
const decision = require('../src/api/services/cohortInterventionExecutionPlanActivationLockDecisionService').serviceInstance;
...
async function setupAuthorizationAndLock(activationAuthId, activationLockId) {
  ...
  const authRecord = {
    ...
    source_activation_readiness_id: 'rd_test_152d',
    source_plan_id: 'pln_test_152d',
    source_dispatcher_id: 'dsp_test_152d',
    source_envelope_id: 'env_test_152d',
    source_auth_id: 'ath_test_152d',
    source_readiness_id: 'rd_test_152d',
    source_approval_id: 'apv_test_152d',
    source_prep_id: 'prep_test_152d',
    source_review_id: 'rev_test_152d',
    source_simulation_id: 'sim_test_152d',
    source_execution_id: 'exec_test_152d',
    cohort_id: 'cohort_test_152d',
    tenant_id: 'tenant_test_152d',
    ...
    source_activation_readiness_hash: 'rd_hash_152d',
    activation_authorization_hash: 'auth_hash_152d',
    evidence_pack_hash: 'pack_hash_152d',
  ...
  const lockRecord = {
    ...
    source_activation_authorization_hash: 'auth_hash_152d',
  ...
  if (!isProdLike) {
    ...
    authEvidenceSvc._mockState.evidence.set(activationAuthId, {
      ...
      lineage_hash_chain_json: {
        ...
        phase143_preparation_id: 'prep_test_152d',
        phase142_review_id: 'rev_test_152d',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_auth
       ...
       VALUES (?, 'rd_test_152d', 'pln_test_152d', 'dsp_test_152d', 'env_test_152d', 'ath_test_152d', 'rd_test_152d', 'apv_test_152d', 'prep_test_152d', 'rev_test_152d', 'sim_test_152d', 'exec_test_152d', 'cohort_test_152d', 'tenant_test_152d', 'SIMULATE_COHORT_PAUSE',
        'FINALIZED', 'AUTHORIZED_NOT_ACTIVE', 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PASS', 'PASS', '{"authorization_mode":"ACTIVATION_AUTHORIZATION_ONLY", "allow_real_activation":false}', '{}', '{}', '{}', '{}', '{}', '{}', ?, ?, 'rd_hash_152d', 'EXECUTION_NOT_ENABLED', 'AUTHORIZATION_FINALIZED_NOT_EXECUTED', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED', 'auth_hash_152d', 'pack_hash_152d')`,
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_lock
       ...
       VALUES (?, ?, 'rd_test_152d', 'pln_test_152d', 'dsp_test_152d', 'env_test_152d', 'ath_test_152d', 'rd_test_152d', 'apv_test_152d', 'prep_test_152d', 'rev_test_152d', 'sim_test_152d', 'exec_test_152d', 'cohort_test_152d', 'tenant_test_152d', 'SIMULATE_COHORT_PAUSE',
        'DRAFT', NULL, 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PENDING', 'PENDING', ?, '{}', '{}', '{}', '{}', '{}', '{"missing_lock_evaluation":true}', ?, ?, 'auth_hash_152d', 'EXECUTION_NOT_ENABLED', 'LOCK_FINALIZED_NOT_EXECUTED', 'FROZEN_IMMUTABLE', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED')`,
      [
        activationLockId,
        activationAuthId,
...
(async () => {
  console.log('=== Smoke 152D: Review Workflow Governance ===\n');

  try {
    const activationAuthId = 'auth_152d_1';
    const activationLockId = 'alk_152d_1';
    await setupAuthorizationAndLock(activationAuthId, activationLockId);

    // 1. Finalization blocks before evaluation
    try {
      await decision.finalizeLock(activationLockId, 'admin');
      assert.fail('Should block finalization when evaluation/decision are not done');
    } catch (e) {
      if (e.message.includes('LOCK_EVALUATION_NOT_COMPLETED') || e.message.includes('LOCK_DECISION_REQUIRED')) {
    ...
    // 3. Record decision
    await decision.recordDecision(activationLockId, 'LOCKED_NOT_ACTIVE', 'Activation lock freeze complete.', 'admin');

    // 4. Build evidence
    await evidenceSvc.buildEvidencePack(activationLockId, 'admin');

    // 5. Finalize
    const { lock } = await decision.finalizeLock(activationLockId, 'admin');
    assert.strictEqual(lock.activation_lock_status, 'FINALIZED');
    assert.strictEqual(lock.execution_capability_status, 'EXECUTION_NOT_ENABLED');
    assert.strictEqual(lock.activation_execution_status, 'LOCK_FINALIZED_NOT_EXECUTED');
    console.log('  PASS: Activation lock finalized successfully with safe non-execution markers.');

    // 6. Block modification after finalization
    try {
      await decision.recordDecision(activationLockId, 'LOCK_BLOCKED_BY_GUARDRAIL', 'Change mind', 'admin');
      assert.fail('Should block modifications on finalized lock');
    } catch (e) {
      if (e.message.includes('LOCK_RECORD_ALREADY_FINALIZED')) {
        console.log('  PASS: Modifications blocked on finalized lock.');
      } else {
        throw e;
      }
    }
    ...
    console.error('FAIL in 152D:', e.message);
    process.exit(1);
  } finally {
