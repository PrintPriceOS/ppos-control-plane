...
const PHASE = '165';
const PHASE_NAME = 'CONTROLLED HIGH-RISK COHORT INTERVENTION ACTIVATION TOKEN REDEMPTION LOCK / PRE-REDEMPTION FREEZE GATE';

const SMOKE_TESTS = [
  { name: `smoke_phase${PHASE}a_activation_token_redemption_lock_schema.js`, label: `Smoke ${PHASE}A: Phase ${PHASE} Schema Validation` },
  { name: `smoke_phase${PHASE}b_create_token_redemption_lock_from_phase164_final_approval.js`, label: `Smoke ${PHASE}B: Create Token Redemption Lock Draft` },
  { name: `smoke_phase${PHASE}c_activation_token_redemption_lock_evaluator_rules.js`, label: `Smoke ${PHASE}C: Activation Token Redemption Lock Evaluator Rules` },
  { name: `smoke_phase${PHASE}d_activation_token_redemption_lock_workflow_governance.js`, label: `Smoke ${PHASE}D: Lock Workflow Governance` },
  ...
  { name: `smoke_phase${PHASE}g_guardrail_redemption_lock_passed_not_redeemed_write_scope.js`, label: `Smoke ${PHASE}G: Guardrails & Safety Boundary Scanner` }
];
...
(async () => {
  ...
  console.log(`--- Validating Phase 164H Dependency ---`);
  try {
    const phase164HPath = path.join(__dirname, 'smoke_phase164h_acceptance_pack_real_db.js');
    const phase164HOutput = execFileSync(process.execPath, ['-r', 'dotenv/config', phase164HPath], {
      env: { ...process.env, ALLOW_SCHEMA_SMOKE_FALLBACK: 'true', ALLOW_SMOKE_FALLBACK: 'true' },
      stdio: 'pipe',
      cwd: path.join(__dirname, '..')
    });
    console.log(phase164HOutput.toString());
    console.log(`  PASS: Phase 164H dependency validated successfully.`);
  } catch (e) {
    console.warn(`  WARN: Phase 164H dependency check failed (non-blocking in mock mode): ${e.message}`);
  }
