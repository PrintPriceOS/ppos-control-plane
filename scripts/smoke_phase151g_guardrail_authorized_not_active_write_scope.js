...
const builder = require('../src/api/services/cohortInterventionExecutionPlanActivationAuthorizationBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionPlanActivationAuthorizationGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionPlanActivationAuthorizationAdmin.js'
];

(async () => {
  console.log('=== Smoke 151G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      try {
        await builder.createAuthorization('non_existent_readiness', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 151 operations!`);
    ...
    console.error('FAIL in 151G:', e.message);
    process.exit(1);
  } finally {
