...
const builder = require('../src/api/services/cohortInterventionExecutionAuthorizationBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionAuthorizationGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionAuthorizationAdmin.js'
];

(async () => {
  console.log('=== Smoke 146G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      // Run dummy authorization builder call
      try {
        await builder.createAuth('non_existent_readiness', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 146 operations!`);
    ...
    console.error('FAIL in 146G:', e.message);
    process.exit(1);
  } finally {
