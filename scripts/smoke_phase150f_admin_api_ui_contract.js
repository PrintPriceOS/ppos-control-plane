...
const adminRouter = require('../src/api/routes/controlledBetaCohortInterventionExecutionPlanActivationReadinessAdmin');

(async () => {
  console.log('=== Smoke 150F: Admin API & UI Contract Verification ===\n');
  ...
  try {
    ...
    const expectedRoutes = [
      '/readiness',
      '/readiness/:activationRdId',
      '/readiness/from-plan/:planId',
      '/readiness/:activationRdId/evaluate',
      '/readiness/:activationRdId/decision',
      '/readiness/:activationRdId/finalize'
    ];
    ...
    const clientPath = path.join(__dirname, '../src/ui/lib/controlledBetaCohortInterventionExecutionPlanActivationReadinessClient.ts');
    ...
    const pagePath = path.join(__dirname, '../src/ui/pages/beta/ControlledBetaCohortInterventionSimulationExecutionPlanActivationReadiness.tsx');
    assert.ok(fs.existsSync(pagePath), 'React manager page file must exist');
    const pageContent = fs.readFileSync(pagePath, 'utf8');
    
    assert.ok(
      pageContent.includes('ACTIVATION READINESS ONLY') ||
      pageContent.includes('READINESS_ONLY') ||
      pageContent.includes('Activation readiness does not') ||
    ...
    console.error('FAIL in 150F:', e.message);
    process.exit(1);
  }
})();
