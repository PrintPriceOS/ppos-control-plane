...
(async () => {
  console.log('=== Smoke 175C: Activation Token Redemption Unlock Risk Officer Countersign Evaluator Rules ===');

  const unlockComplianceWitnessId = 'cwn_smoke_175c';
  const unlockFinalHumanAuthorizationSealId = 'fhas_smoke_175c';
  const unlockDualControlAuthorizationId = 'dcau_smoke_175c';
  const unlockOperatorAttestationId = 'oatt_smoke_175c';
  const unlockPreExecutionFreezeId = 'freeze_smoke_175c';
  const unlockSealId = 'seal_smoke_175c';
  const finalReviewId = 'frev_smoke_175c';
  const approvalId = 'apv_smoke_175c';
  const eligibilityId = 'elig_smoke_175c';
  const lockId = 'lock_smoke_175c';
  const finalApvId = 'fapv_smoke_175c';
  const envId = 'env_smoke_175c';
  const authId = 'auth_smoke_175c';
  const readinessId = 'readiness_smoke_175c';
  const issuanceId = 'issuance_smoke_175c';
  ...
  try {
    ...
    await decisionSvc.recordRiskOfficer(tempId, 'user_diana', 'risk_officer', 'Risk officer checks done', 'admin');
    ...
    const evalResult = await evaluator.evaluateUnlockRiskOfficerCountersign(tempId, confirmations, 'admin');
    assert.ok(evalResult.tokenRedemptionUnlockRiskOfficerCountersign, 'Evaluation result missing record');
    assert.ok(evalResult.rules.length >= 29, `Expected at least 29 rules evaluated, got ${evalResult.rules.length}`);
    ...
    console.log('  PASS: Evaluator ran successfully with 14 confirmations.');
    ...
    console.error('FAIL in 175C:', e.message, e.stack);
    process.exit(1);
  } finally {
