...
async function setupLockAndDecision(activationLockId, activationDecisionId) {
  ...
  const lockRecord = {
    ...
    source_activation_auth_id: 'auth_test_153e',
    source_activation_readiness_id: 'rd_test_153e',
    source_plan_id: 'pln_test_153e',
    source_dispatcher_id: 'dsp_test_153e',
    source_envelope_id: 'env_test_153e',
    source_auth_id: 'ath_test_153e',
    source_readiness_id: 'rd_test_153e',
    source_approval_id: 'apv_test_153e',
    source_prep_id: 'prep_test_153e',
    source_review_id: 'rev_test_153e',
    source_simulation_id: 'sim_test_153e',
    source_execution_id: 'exec_test_153e',
    cohort_id: 'cohort_test_153e',
    tenant_id: 'tenant_test_153e',
    ...
    source_activation_authorization_hash: 'auth_hash_153e',
    activation_lock_hash: 'lock_hash_153e',
    freeze_package_hash: 'lock_hash_153e',
    evidence_pack_hash: 'pack_hash_153e',
  ...
  const decisionRecord = {
    ...
    source_activation_lock_hash: 'lock_hash_153e',
    source_freeze_package_hash: 'lock_hash_153e',
  ...
  if (!isProdLike) {
    ...
    lockEvidenceSvc._mockState.evidence.set(activationLockId, {
      ...
      lineage_hash_chain_json: {
        ...
        phase143_preparation_id: 'prep_test_153e',
        phase142_review_id: 'rev_test_153e',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_lock
       ...
       VALUES (?, 'auth_test_153e', 'rd_test_153e', 'pln_test_153e', 'dsp_test_153e', 'env_test_153e', 'ath_test_153e', 'rd_test_153e', 'apv_test_153e', 'prep_test_153e', 'rev_test_153e', 'sim_test_153e', 'exec_test_153e', 'cohort_test_153e', 'tenant_test_153e', 'SIMULATE_COHORT_PAUSE',
        'FINALIZED', 'LOCKED_NOT_ACTIVE', 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PASS', 'PASS', '{"lock_mode":"PRE_EXECUTION_FREEZE_ONLY", "allow_real_activation":false}', '{}', '{}', '{}', '{}', '{}', '{}', ?, ?, 'auth_hash_153e', 'EXECUTION_NOT_ENABLED', 'LOCK_FINALIZED_NOT_EXECUTED', 'FROZEN_IMMUTABLE', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED', 'lock_hash_153e', 'lock_hash_153e', 'pack_hash_153e')`,
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_decision
       ...
       VALUES (?, ?, 'auth_test_153e', 'rd_test_153e', 'pln_test_153e', 'dsp_test_153e', 'env_test_153e', 'ath_test_153e', 'rd_test_153e', 'apv_test_153e', 'prep_test_153e', 'rev_test_153e', 'sim_test_153e', 'exec_test_153e', 'cohort_test_153e', 'tenant_test_153e', 'SIMULATE_COHORT_PAUSE',
        'DRAFT', NULL, 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PENDING', 'PENDING', ?, '{}', '{}', '{}', '{}', '{}', '{"missing_decision_evaluation":true}', ?, ?, 'lock_hash_153e', 'lock_hash_153e', 'EXECUTION_NOT_ENABLED', 'GO_DECISION_FINALIZED_NOT_EXECUTED', 'FROZEN_IMMUTABLE', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED')`,
      [
...
(async () => {
  console.log('=== Smoke 153E: Evidence Pack Builder & Lineage ===\n');

  try {
    const activationLockId = 'lock_153e_1';
    const activationDecisionId = 'adc_153e_1';
    ...
    if (!isProdLike) {
      decisionBuilder._mockState.rules.set(activationDecisionId, [sensitiveRule]);
    } else {
      await db.query(
        `INSERT INTO cb_cohort_intervention_activation_decision_rules
         (rule_id, activation_decision_id, check_type, severity, description)
         VALUES (?, ?, ?, ?, ?)`,
        [sensitiveRule.rule_id, activationDecisionId, sensitiveRule.check_type, sensitiveRule.severity, sensitiveRule.description]
      );
    }

    // Build evidence pack
    const runRes = await evidenceSvc.buildEvidencePack(activationDecisionId, 'admin');
    const { evidence_pack_hash, lineage_hash_chain } = runRes;
    assert.ok(evidence_pack_hash);

    const evidenceRecord = await evidenceSvc.getEvidence(activationDecisionId);
    ...
    // Verify v153.0 schema version
    assert.strictEqual(payload.evidence_schema_version, '153.0');
    console.log('  PASS: Evidence schema version is 153.0.');
    ...
    assert.strictEqual(lineage_hash_chain.phase153_activation_decision_id, activationDecisionId);
    assert.strictEqual(lineage_hash_chain.phase152_source_activation_lock_hash, 'lock_hash_153e');
    ...
    console.error('FAIL in 153E:', e.message);
    process.exit(1);
  } finally {
