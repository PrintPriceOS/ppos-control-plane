...
const builder = require('../src/api/services/cohortInterventionExecutionReadinessBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionReadinessGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionReadinessAdmin.js'
];

(async () => {
  console.log('=== Smoke 145G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      try {
        await builder.createReadiness('non_existent_approval', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 145 operations!`);
    ...
    console.error('FAIL in 145G:', e.message);
    process.exit(1);
  } finally {
