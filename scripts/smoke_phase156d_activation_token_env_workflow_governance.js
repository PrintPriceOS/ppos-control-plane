...
const decision = require('../src/api/services/cohortInterventionExecutionPlanActivationTokenEnvDecisionService').serviceInstance;
...
async function setupTokenAuthAndTokenEnv(activationTokenAuthId, activationTokenEnvId) {
  ...
  const tokenAuthRecord = {
    ...
    source_activation_handoff_id: 'ahf_test_156d',
    source_activation_decision_id: 'dec_test_156d',
    source_activation_lock_id: 'lock_test_156d',
    source_activation_auth_id: 'auth_test_156d',
    source_activation_readiness_id: 'rd_test_156d',
    source_plan_id: 'pln_test_156d',
    source_dispatcher_id: 'dsp_test_156d',
    source_envelope_id: 'env_test_156d',
    source_auth_id: 'ath_test_156d',
    source_readiness_id: 'rd_test_156d',
    source_approval_id: 'apv_test_156d',
    source_prep_id: 'prep_test_156d',
    source_review_id: 'rev_test_156d',
    source_simulation_id: 'sim_test_156d',
    source_execution_id: 'exec_test_156d',
    cohort_id: 'cohort_test_156d',
    tenant_id: 'tenant_test_156d',
    ...
    source_activation_handoff_hash: 'handoff_hash_156d',
    source_token_material_hash: 'token_material_hash_156d',
    source_freeze_package_hash: 'lock_hash_156d',
    activation_token_auth_hash: 'token_auth_hash_156d',
    token_auth_evidence_pack_hash: 'pack_hash_156d',
    evidence_pack_hash: 'pack_hash_156d',
  ...
  const tokenEnvRecord = {
    ...
    source_activation_token_auth_hash: 'token_auth_hash_156d',
    source_token_material_hash: 'token_material_hash_156d',
    source_freeze_package_hash: 'lock_hash_156d',
  ...
  if (!isProdLike) {
    ...
    tokenAuthEvidenceSvc._mockState.evidence.set(activationTokenAuthId, {
      evidence_pack_hash: 'pack_hash_156d',
      evidence_payload_json: { evidence_schema_version: '155.0', write_scope_attestation: writeScope155 },
      lineage_hash_chain_json: {
        phase155_activation_token_auth_id: activationTokenAuthId,
        phase154_activation_handoff_id: 'ahf_test_156d',
        phase154_source_activation_handoff_hash: 'handoff_hash_d',
        phase153_activation_decision_id: 'dec_test_156d',
        ...
        phase143_preparation_id: 'prep_test_156d',
        phase142_review_id: 'rev_test_156d',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_auth
       ...
       VALUES (?, 'ahf_test_156d', 'dec_test_156d', 'lock_test_156d', 'auth_test_156d', 'rd_test_156d', 'pln_test_156d', 'dsp_test_156d', 'env_test_156d', 'ath_test_156d', 'rd_test_156d', 'apv_test_156d', 'prep_test_156d', 'rev_test_156d', 'sim_test_156d', 'exec_test_156d', 'cohort_test_156d', 'tenant_test_156d', 'SIMULATE_COHORT_PAUSE',
        'FINALIZED', 'AUTHORIZED_NOT_ISSUED', 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PASS', 'PASS', '{"token_auth_mode":"TOKEN_ISSUANCE_AUTHORIZATION_ONLY", "allow_token_issue":false, "token_status":"PREPARED_NOT_ISSUED", "token_issuance_status":"AUTHORIZED_NOT_ISSUED", "token_redeemable":false}', '{}', '{}', '{}', '{}', '{}', '{}', ?, ?, 'handoff_hash_156d', 'token_material_hash_156d', 'lock_hash_156d', 'EXECUTION_NOT_ENABLED', 'TOKEN_AUTH_FINALIZED_NOT_EXECUTED', 'FROZEN_IMMUTABLE', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED', 'token_auth_hash_156d', 'pack_hash_156d', 'pack_hash_156d')`,
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_auth_evidence
       ...
       VALUES (?, ?, '155.0', 'pack_hash_156d', ?, ?)`,
      [
        'ae_' + activationTokenAuthId,
        activationTokenAuthId,
        JSON.stringify({ evidence_schema_version: '155.0', write_scope_attestation: writeScope155 }),
        JSON.stringify({
          phase155_activation_token_auth_id: activationTokenAuthId,
          phase154_activation_handoff_id: 'ahf_test_156d',
          phase154_source_activation_handoff_hash: 'handoff_hash_d',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_env
       ...
       VALUES (?, ?, 'ahf_test_156d', 'dec_test_156d', 'lock_test_156d', 'auth_test_156d', 'rd_test_156d', 'pln_test_156d', 'dsp_test_156d', 'env_test_156d', 'ath_test_156d', 'rd_test_156d', 'apv_test_156d', 'prep_test_156d', 'rev_test_156d', 'sim_test_156d', 'exec_test_156d', 'cohort_test_156d', 'tenant_test_156d', 'SIMULATE_COHORT_PAUSE',
        'DRAFT', NULL, 'LOW', 'HIGH', 35.0, 80.0, 95.0, 'PENDING', 'PENDING', ?, '{}', '{}', '{}', '{}', '{}', '{"missing_token_env_evaluation":true}', ?, ?, 'token_auth_hash_156d', 'token_material_hash_156d', 'lock_hash_156d', 'EXECUTION_NOT_ENABLED', 'TOKEN_ENV_FINALIZED_NOT_EXECUTED', 'FROZEN_IMMUTABLE', 'NOT_EXECUTABLE', 'NO_REAL_JOB_CREATED', 'NO_QUEUE_DISPATCHED', 'ZERO_RUNTIME_MUTATION_CONFIRMED')`,
      [
...
(async () => {
  console.log('=== Smoke 156D: Review Workflow Governance ===\n');

  try {
    const activationTokenAuthId = 'ath_156d_1';
    const activationTokenEnvId = 'ate_156d_1';
    await setupTokenAuthAndTokenEnv(activationTokenAuthId, activationTokenEnvId);

    // 1. Finalization blocks before evaluation
    try {
      await decision.finalizeTokenEnv(activationTokenEnvId, 'admin');
      assert.fail('Should block finalization when evaluation/decision are not done');
    } catch (e) {
      if (e.message.includes('TOKEN_ENV_EVALUATION_NOT_COMPLETED') || e.message.includes('TOKEN_ENV_RESULT_REQUIRED')) {
    ...
    // 3. Record decision
    await decision.recordDecision(activationTokenEnvId, 'ENVELOPE_PREPARED_NOT_ISSUED', 'Activation token envelope prepared complete.', 'admin');

    // 4. Build evidence
    await evidenceSvc.buildEvidencePack(activationTokenEnvId, 'admin');

    // 5. Finalize
    const { tokenEnv: finalRecord } = await decision.finalizeTokenEnv(activationTokenEnvId, 'admin');
    assert.strictEqual(finalRecord.activation_token_env_status, 'FINALIZED');
    assert.strictEqual(finalRecord.execution_capability_status, 'EXECUTION_NOT_ENABLED');
    assert.strictEqual(finalRecord.activation_execution_status, 'TOKEN_ENV_FINALIZED_NOT_EXECUTED');
    console.log('  PASS: Activation token envelope finalized successfully with safe non-execution markers.');

    // 6. Block modification after finalization
    try {
      await decision.recordDecision(activationTokenEnvId, 'ENVELOPE_BLOCKED_BY_GUARDRAIL', 'Change mind', 'admin');
      assert.fail('Should block modifications on finalized token envelope');
    } catch (e) {
      if (e.message.includes('TOKEN_ENV_RECORD_ALREADY_FINALIZED')) {
        console.log('  PASS: Modifications blocked on finalized token envelope.');
      } else {
        throw e;
      }
    }
    ...
    console.error('FAIL in 156D:', e.message);
    process.exit(1);
  } finally {
