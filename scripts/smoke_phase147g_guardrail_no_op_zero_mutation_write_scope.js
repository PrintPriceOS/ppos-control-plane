...
const builder = require('../src/api/services/cohortInterventionExecutionEnvelopeBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionEnvelopeGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionEnvelopeAdmin.js'
];

(async () => {
  console.log('=== Smoke 147G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      // Run dummy envelope builder call
      try {
        await builder.createEnvelope('non_existent_auth', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 147 operations!`);
    ...
    console.error('FAIL in 147G:', e.message);
    process.exit(1);
  } finally {
