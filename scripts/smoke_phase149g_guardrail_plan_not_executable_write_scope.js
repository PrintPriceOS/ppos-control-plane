...
const builder = require('../src/api/services/cohortInterventionExecutionPlanBuilderService').serviceInstance;
...
const FILES_TO_SCAN = [
  ...
  'src/api/services/cohortInterventionExecutionPlanGuardrailService.js',
  'src/api/routes/controlledBetaCohortInterventionExecutionPlanAdmin.js'
];

(async () => {
  console.log('=== Smoke 149G: Guardrails & Safety Boundary Scanner ===\n');
  ...
  try {
    ...
    if (isProdLike) {
      ...
      // Run dummy plan builder call
      try {
        await builder.createPlan('non_existent_dispatcher', 'admin');
      } catch (err) {
        // expected failure
      }
      ...
      for (const table of checkTables) {
        try {
          const rows = await db.query(`SELECT COUNT(*) as count FROM ${table}`);
          assert.strictEqual(rows[0].count, countsBefore[table], `Table ${table} mutated during Phase 149 operations!`);
    ...
    console.error('FAIL in 149G:', e.message);
    process.exit(1);
  } finally {
