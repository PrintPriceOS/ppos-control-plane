...
async function setupTokenFinalApvAndStaging(apvId, stagingId, status = 'FINALIZED', result = 'FINAL_APPROVED_NOT_ISSUED', stagingConfig = {}) {
  ...
  const activeStagingConfig = { ...defaultStagingConfig, ...stagingConfig };

  const apvRecord = {
    activation_token_final_apv_id: apvId,
    source_activation_token_env_id: 'ate_test_158c',
    source_activation_token_auth_id: 'ath_test_158c',
    source_activation_handoff_id: 'ahf_test_158c',
    source_activation_decision_id: 'dec_test_158c',
    source_activation_lock_id: 'lock_test_158c',
    source_activation_auth_id: 'auth_test_158c',
    source_activation_readiness_id: 'rd_test_158c',
    source_plan_id: 'pln_test_158c',
    source_dispatcher_id: 'dsp_test_158c',
    source_envelope_id: 'env_test_158c',
    source_auth_id: 'ath_test_158c',
    source_readiness_id: 'rd_test_158c',
    source_approval_id: 'apv_test_158c',
    source_prep_id: 'prep_test_158c',
    source_review_id: 'rev_test_158c',
    source_simulation_id: 'sim_test_158c',
    source_execution_id: 'exec_test_158c',
    cohort_id: 'cohort_test_158c',
    tenant_id: 'tenant_test_158c',
    ...
    source_activation_token_env_hash: 'token_env_hash_158c',
    source_token_material_hash: 'token_material_hash_158c',
    source_freeze_package_hash: 'lock_hash_158c',
    activation_token_final_apv_hash: 'apv_hash_158c',
    token_final_apv_evidence_pack_hash: 'pack_hash_158c',
    evidence_pack_hash: 'pack_hash_158c',
  ...
  const stagingRecord = {
    ...
    canary_envelope_json: activeStagingConfig,
    ...
    source_activation_token_final_apv_hash: 'apv_hash_158c',
    source_token_material_hash: 'token_material_hash_158c',
    source_freeze_package_hash: 'lock_hash_158c',
  ...
  if (!isProdLike) {
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_final_apv
       ...
       VALUES (?, ?, 'ath_test_158c', 'ahf_test_158c', 'dec_test_158c', 'lock_test_158c', 'auth_test_158c', 'rd_test_158c', 'pln_test_158c', 'dsp_test_158c', 'env_test_158c', 'ath_test_158c', 'rd_test_158c', 'apv_test_158c', 'prep_test_158c', 'rev_test_158c', 'sim_test_158c', 'exec_test_158c', 'cohort_test_158c', 'tenant_test_158c', 'SIMULATE_COHORT_PAUSE',
    ...
    await db.query(
      `INSERT INTO cb_cohort_intervention_activation_token_staging
       (activation_token_staging_id, source_activation_token_final_apv_id, source_activation_token_env_id, source_activation_token_auth_id, source_activation_handoff_id, source_activation_decision_id, source_activation_lock_id, source_activation_auth_id, source_activation_readiness_id, source_plan_id, source_dispatcher_id, source_envelope_id, source_auth_id, source_readiness_id, source_approval_id, source_prep_id, source_review_id, source_simulation_id, source_execution_id, cohort_id, tenant_id, simulation_type,
        ...
        execution_capability_status, activation_execution_status, package_freeze_status, plan_executable_status, job_creation_status, queue_dispatch_status, runtime_mutation_status)
       VALUES (?, ?, 'ate_test_158c', 'ath_test_158c', 'ahf_test_158c', 'dec_test_158c', 'lock_test_158c', 'auth_test_158c', 'rd_test_158c', 'pln_test_158c', 'dsp_test_158c', 'env_test_158c', 'ath_test_158c', 'rd_test_158c', 'apv_test_158c', 'prep_test_158c', 'rev_test_158c', 'sim_test_158c', 'exec_test_158c', 'cohort_test_158c', 'tenant_test_158c', 'SIMULATE_COHORT_PAUSE',
      ...
      [
        stagingId,
        apvId,
        JSON.stringify(activeStagingConfig),
        JSON.stringify(nonExecution158),
        JSON.stringify(writeScope158),
        'apv_hash_158c',
        'token_material_hash_158c',
        'lock_hash_158c'
      ]
    );
  }
...
(async () => {
  console.log('=== Smoke 158C: Activation Token Staging Evaluator Rules ===\n');

  try {
    // 1. Positive: evaluate ready record with all metrics passing
    const apvId1 = 'apv_158c_1';
    const stagingId1 = 'ats_158c_1';
    await setupTokenFinalApvAndStaging(apvId1, stagingId1);
    
    const passed = await evaluator.evaluateTokenStaging(stagingId1, {
      security_officer_confirmed: true,
      compliance_officer_confirmed: true,
      operations_director_confirmed: true
    }, 'admin');

    const rules = await tokenStagingBuilder.getRules(stagingId1);
    console.log('Evaluated rules list for stagingId1:', JSON.stringify(rules, null, 2));

    assert.strictEqual(passed.success, true);
    let record = await tokenStagingBuilder.getTokenStaging(stagingId1);
    assert.strictEqual(record.activation_token_staging_status, 'EVALUATED');
    assert.strictEqual(record.activation_token_staging_result, 'STAGED_NOT_ISSUED');
    console.log('  PASS: Evaluated token final staging record successfully.');

    // 2. Negative: fail check if security officer signature is missing
    const apvId2 = 'apv_158c_2';
    const stagingId2 = 'ats_158c_2';
    await setupTokenFinalApvAndStaging(apvId2, stagingId2);
    
    const passedFail = await evaluator.evaluateTokenStaging(stagingId2, {
    ...
    record = await tokenStagingBuilder.getTokenStaging(stagingId2);
    assert.strictEqual(record.activation_token_staging_status, 'BLOCKED');
    assert.strictEqual(record.activation_token_staging_result, 'STAGING_BLOCKED_BY_GUARDRAIL');
    console.log('  PASS: Correctly failed evaluation when security officer confirmation is missing.');

    console.log('\nSmoke 158C: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 158C:', e.message);
    process.exit(1);
  } finally {
