...
(async () => {
  console.log('=== Smoke 151A: Phase 151 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_auth',
      'cb_cohort_intervention_activation_auth_rules',
      'cb_cohort_intervention_activation_auth_evidence',
      'cb_cohort_intervention_activation_auth_audits'
    ];
    ...
    // Validate key columns on main activation authorization table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_auth'`
    );
    ...
    for (const col of [
      ...
      'activation_auth_status', 'activation_auth_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_auth.${col}' exists.`);
    }

    console.log('\nSmoke 151A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 151A:', e.message);
    process.exit(1);
  } finally {
