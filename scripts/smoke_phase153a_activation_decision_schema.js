...
(async () => {
  console.log('=== Smoke 153A: Phase 153 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_decision',
      'cb_cohort_intervention_activation_decision_rules',
      'cb_cohort_intervention_activation_decision_evidence',
      'cb_cohort_intervention_activation_decision_audits'
    ];
    ...
    // Validate key columns on main activation decision table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_decision'`
    );
    ...
    for (const col of [
      ...
      'activation_decision_status', 'activation_decision_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_decision.${col}' exists.`);
    }

    console.log('\nSmoke 153A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 153A:', e.message);
    process.exit(1);
  } finally {
