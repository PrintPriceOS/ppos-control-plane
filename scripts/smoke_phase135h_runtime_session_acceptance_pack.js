...
console.log('=== Phase 135 Runtime Session Acceptance Pack (135H) ===\n');
...
(async () => {
  ...
  const smokes = [
    'smoke_phase135a_runtime_session_schema.js',
    'smoke_phase135b_runtime_session_service.js',
    'smoke_phase135c_runtime_session_readiness.js',
    'smoke_phase135d_runtime_session_limits_scope.js',
    'smoke_phase135e_runtime_session_lifecycle_guardrails.js',
    'smoke_phase135f_runtime_session_admin_api_ui.js',
    'smoke_phase135g_runtime_session_evidence_pack.js',
    // Run Phase 134H as dependency if DB is reachable
    ...(hasDb ? ['smoke_phase134h_invite_acceptance_acceptance_pack.js'] : [])
  ];
  ...
  const svcPath = path.join(__dirname, '../src/api/services/controlledBetaRuntimeSessionService.js');
  ...
  console.log(`\nSmoke 135H: Finished execution. ${passed} passed, ${failed} failed`);
  ...
  if (failed > 0 || failedSmokes.length > 0) {
    ...
    console.error('\nFATAL: Phase 135 Acceptance Pack Failed.');
    process.exit(1);
  } else {
    console.log('\nSUCCESS: Phase 135 Acceptance Pack Passed.');
    process.exit(0);
  }
})().catch(err => {
  console.error('FATAL error in 135H:', err);
  process.exit(1);
});
