...
(async () => {
  console.log('=== Smoke 152A: Phase 152 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_lock',
      'cb_cohort_intervention_activation_lock_rules',
      'cb_cohort_intervention_activation_lock_evidence',
      'cb_cohort_intervention_activation_lock_audits'
    ];
    ...
    // Validate key columns on main activation lock table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_lock'`
    );
    ...
    for (const col of [
      ...
      'activation_lock_status', 'activation_lock_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_lock.${col}' exists.`);
    }

    console.log('\nSmoke 152A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 152A:', e.message);
    process.exit(1);
  } finally {
