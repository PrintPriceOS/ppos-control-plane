...
(async () => {
  console.log('=== Smoke 149A: Phase 149 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_exec_plan',
      'cb_cohort_intervention_plan_rules',
      'cb_cohort_intervention_plan_evidence',
      'cb_cohort_intervention_plan_audits'
    ];
    ...
    // Validate key columns on main execution plan table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_exec_plan'`
    );
    ...
    for (const col of [
      ...
      'plan_status', 'plan_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_exec_plan.${col}' exists.`);
    }

    console.log('\nSmoke 149A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 149A:', e.message);
    process.exit(1);
  } finally {
