...
(async () => {
  console.log('=== Smoke 155A: Phase 155 Schema Validation ===\n');
  const isProdLike = (process.env.NODE_ENV === 'production' || !!process.env.DATABASE_URL || process.env.CI_PRODUCTION_SMOKE === 'true') && process.env.DB_UNREACHABLE !== 'true';

  try {
    const REQUIRED_TABLES = [
      'cb_cohort_intervention_activation_token_auth',
      'cb_cohort_intervention_activation_token_auth_rules',
      'cb_cohort_intervention_activation_token_auth_evidence',
      'cb_cohort_intervention_activation_token_auth_audits'
    ];
    ...
    // Validate key columns on main activation token auth table
    const cols = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'cb_cohort_intervention_activation_token_auth'`
    );
    ...
    for (const col of [
      ...
      'activation_token_auth_status', 'activation_token_auth_result', 'projected_impact_score', 'rollback_feasibility_score',
      ...
      console.log(`  PASS: Column 'cb_cohort_intervention_activation_token_auth.${col}' exists.`);
    }

    console.log('\nSmoke 155A: Passed.');
    process.exit(0);
  } catch (e) {
    console.error('FAIL in 155A:', e.message);
    process.exit(1);
  } finally {
