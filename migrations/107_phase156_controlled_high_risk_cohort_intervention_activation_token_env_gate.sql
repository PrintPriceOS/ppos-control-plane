-- Migration: 107_phase156_controlled_high_risk_cohort_intervention_activation_token_env_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_env (
  activation_token_env_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_token_env_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_token_env_result VARCHAR(128) NULL,
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_env_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_token_env_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(100) NOT NULL,
  severity VARCHAR(50) NOT NULL,
  description TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_156_rules_dec
    FOREIGN KEY (activation_token_env_id)
    REFERENCES cb_cohort_intervention_activation_token_env(activation_token_env_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_env_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_156_evidence_dec
    FOREIGN KEY (activation_token_env_id)
    REFERENCES cb_cohort_intervention_activation_token_env(activation_token_env_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_env_audits (
