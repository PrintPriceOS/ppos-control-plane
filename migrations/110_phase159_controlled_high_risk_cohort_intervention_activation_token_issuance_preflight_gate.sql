-- Migration: 110_phase159_controlled_high_risk_cohort_intervention_activation_token_issuance_preflight_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_preflight (
  activation_token_preflight_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_token_preflight_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_token_preflight_result VARCHAR(128) NULL,
  ...
  token_preflight_summary_json JSON NULL,
  impact_review_json JSON NULL,
  rollback_review_json JSON NULL,
  guardrail_review_json JSON NULL,
  token_preflight_rules_json JSON NULL,
  token_preflight_blockers_json JSON NULL,
  ...
  activation_token_preflight_hash VARCHAR(256) NULL,
  token_preflight_evidence_pack_hash VARCHAR(256) NULL,
  evidence_pack_hash VARCHAR(256) NULL,
  lineage_hash_chain_json JSON NULL,
  preflight_signatures_json JSON NULL,
  preflight_metadata_json JSON NULL,
  execution_capability_status VARCHAR(64) NOT NULL DEFAULT 'EXECUTION_NOT_ENABLED',
  activation_execution_status VARCHAR(128) NOT NULL DEFAULT 'TOKEN_PREFLIGHT_FINALIZED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_preflight_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_token_preflight_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(128) NOT NULL,
  severity VARCHAR(32) NOT NULL DEFAULT 'INFO',
  description TEXT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_preflight_rules_preflight_id (activation_token_preflight_id)
);

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_preflight_evidence (
  evidence_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_token_preflight_id VARCHAR(64) NOT NULL,
  evidence_schema_version VARCHAR(16) NOT NULL DEFAULT '159.0',
  evidence_pack_hash VARCHAR(256) NULL,
  evidence_payload_json JSON NULL,
  lineage_hash_chain_json JSON NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_preflight_evidence_preflight_id (activation_token_preflight_id)
);

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_preflight_audits (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_preflight_audits_preflight_id (activation_token_preflight_id)
);
