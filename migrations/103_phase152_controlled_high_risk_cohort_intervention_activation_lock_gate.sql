-- Migration: 103_phase152_controlled_high_risk_cohort_intervention_activation_lock_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_lock (
  activation_lock_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_lock_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_lock_result VARCHAR(128) NULL,
  ...
  lock_summary_json JSON NULL,
  impact_review_json JSON NULL,
  rollback_review_json JSON NULL,
  guardrail_review_json JSON NULL,
  lock_rules_json JSON NULL,
  lock_blockers_json JSON NULL,
  non_execution_attestation_json JSON NULL,
  write_scope_attestation_json JSON NULL,
  source_activation_authorization_hash VARCHAR(128) NULL,
  activation_lock_hash VARCHAR(128) NULL,
  freeze_package_hash VARCHAR(128) NULL,
  lock_evidence_pack_hash VARCHAR(128) NULL,
  ...
  activation_execution_status VARCHAR(64) NOT NULL DEFAULT 'LOCK_FINALIZED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_lock_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_lock_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(100) NOT NULL,
  severity VARCHAR(50) NOT NULL,
  description TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_152_rules_lock
    FOREIGN KEY (activation_lock_id)
    REFERENCES cb_cohort_intervention_activation_lock(activation_lock_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_lock_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_152_evidence_lock
    FOREIGN KEY (activation_lock_id)
    REFERENCES cb_cohort_intervention_activation_lock(activation_lock_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_lock_audits (
