-- Migration: 097_phase146_controlled_high_risk_cohort_intervention_execution_authorization_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_exec_auth (
  auth_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  auth_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  auth_decision VARCHAR(128) NULL,
  ...
  source_readiness_hash VARCHAR(128) NULL,
  source_readiness_evidence_pack_hash VARCHAR(128) NULL,
  auth_result_hash VARCHAR(128) NULL,
  ...
  execution_authorization_status VARCHAR(64) NOT NULL DEFAULT 'EXECUTION_AUTHORIZED_NOT_ACTIVE',
  auth_execution_status VARCHAR(64) NOT NULL DEFAULT 'AUTHORIZATION_APPROVED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_exec_auth_rules (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_146_rules_auth
    FOREIGN KEY (auth_id)
    REFERENCES cb_cohort_intervention_exec_auth(auth_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_exec_auth_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_146_evidence_auth
    FOREIGN KEY (auth_id)
    REFERENCES cb_cohort_intervention_exec_auth(auth_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_exec_auth_audits (
