-- Migration: 102_phase151_controlled_high_risk_cohort_intervention_execution_plan_activation_authorization_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_auth (
  activation_auth_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_auth_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_auth_result VARCHAR(128) NULL,
  ...
  auth_summary_json JSON NULL,
  impact_review_json JSON NULL,
  rollback_review_json JSON NULL,
  guardrail_review_json JSON NULL,
  auth_rules_json JSON NULL,
  auth_blockers_json JSON NULL,
  non_execution_attestation_json JSON NULL,
  write_scope_attestation_json JSON NULL,
  source_activation_readiness_hash VARCHAR(128) NULL,
  activation_authorization_hash VARCHAR(128) NULL,
  authorization_evidence_pack_hash VARCHAR(128) NULL,
  ...
  activation_execution_status VARCHAR(64) NOT NULL DEFAULT 'AUTHORIZATION_FINALIZED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_auth_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_auth_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(100) NOT NULL,
  severity VARCHAR(50) NOT NULL,
  description TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_151_rules_auth
    FOREIGN KEY (activation_auth_id)
    REFERENCES cb_cohort_intervention_activation_auth(activation_auth_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_auth_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_151_evidence_auth
    FOREIGN KEY (activation_auth_id)
    REFERENCES cb_cohort_intervention_activation_auth(activation_auth_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_auth_audits (
