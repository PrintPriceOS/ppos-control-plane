-- Migration: 104_phase153_controlled_high_risk_cohort_intervention_final_activation_go_no_go_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_decision (
  activation_decision_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_decision_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_decision_result VARCHAR(128) NULL,
  ...
  decision_blockers_json JSON NULL,
  non_execution_attestation_json JSON NULL,
  write_scope_attestation_json JSON NULL,
  source_activation_lock_hash VARCHAR(128) NULL,
  source_freeze_package_hash VARCHAR(128) NULL,
  activation_decision_hash VARCHAR(128) NULL,
  decision_evidence_pack_hash VARCHAR(128) NULL,
  evidence_pack_hash VARCHAR(128) NULL,
  lineage_hash_chain_json JSON NULL,
  decision_rationale_json JSON NULL,
  execution_capability_status VARCHAR(64) NOT NULL DEFAULT 'EXECUTION_NOT_ENABLED',
  activation_execution_status VARCHAR(64) NOT NULL DEFAULT 'GO_DECISION_FINALIZED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_decision_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_decision_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(100) NOT NULL,
  severity VARCHAR(50) NOT NULL,
  description TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_153_rules_dec
    FOREIGN KEY (activation_decision_id)
    REFERENCES cb_cohort_intervention_activation_decision(activation_decision_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_decision_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_153_evidence_dec
    FOREIGN KEY (activation_decision_id)
    REFERENCES cb_cohort_intervention_activation_decision(activation_decision_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_decision_audits (
