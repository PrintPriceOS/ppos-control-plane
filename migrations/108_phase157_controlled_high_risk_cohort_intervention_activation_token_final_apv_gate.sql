-- Migration: 108_phase157_controlled_high_risk_cohort_intervention_activation_token_final_apv_gate
-- Up

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_final_apv (
  activation_token_final_apv_id VARCHAR(64) NOT NULL PRIMARY KEY,
  ...
  activation_token_final_apv_status VARCHAR(64) NOT NULL DEFAULT 'DRAFT',
  activation_token_final_apv_result VARCHAR(128) NULL,
  ...
  token_final_apv_summary_json JSON NULL,
  impact_review_json JSON NULL,
  rollback_review_json JSON NULL,
  guardrail_review_json JSON NULL,
  token_final_apv_rules_json JSON NULL,
  token_final_apv_blockers_json JSON NULL,
  non_execution_attestation_json JSON NULL,
  write_scope_attestation_json JSON NULL,
  source_activation_token_env_hash VARCHAR(128) NULL,
  source_token_material_hash VARCHAR(128) NULL,
  source_freeze_package_hash VARCHAR(128) NULL,
  activation_token_final_apv_hash VARCHAR(128) NULL,
  token_final_apv_evidence_pack_hash VARCHAR(128) NULL,
  evidence_pack_hash VARCHAR(128) NULL,
  lineage_hash_chain_json JSON NULL,
  security_chair_signature_json JSON NULL,
  final_approval_rationale_json JSON NULL,
  execution_capability_status VARCHAR(64) NOT NULL DEFAULT 'EXECUTION_NOT_ENABLED',
  activation_execution_status VARCHAR(64) NOT NULL DEFAULT 'TOKEN_FINAL_APPROVAL_FINALIZED_NOT_EXECUTED',
...
CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_final_apv_rules (
  rule_id VARCHAR(64) NOT NULL PRIMARY KEY,
  activation_token_final_apv_id VARCHAR(64) NOT NULL,
  check_type VARCHAR(100) NOT NULL,
  severity VARCHAR(50) NOT NULL,
  description TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_157_rules_dec
    FOREIGN KEY (activation_token_final_apv_id)
    REFERENCES cb_cohort_intervention_activation_token_final_apv(activation_token_final_apv_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_final_apv_evidence (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_157_evidence_dec
    FOREIGN KEY (activation_token_final_apv_id)
    REFERENCES cb_cohort_intervention_activation_token_final_apv(activation_token_final_apv_id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS cb_cohort_intervention_activation_token_final_apv_audits (
